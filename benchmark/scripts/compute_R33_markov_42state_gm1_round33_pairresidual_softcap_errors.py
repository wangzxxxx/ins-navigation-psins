...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round33_pairresidual_softcap.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R33_markov_42state_gm1_round33_pairresidual_softcap_param_errors.json'
...
meth = load(METHOD, 'meth_R33')
src = load(SRC, 'src_R33')
res = meth.run_method()
clbt = res[0]
extra = res[4]
...
OUT.write_text(json.dumps({'method': 'R33_markov_42state_gm1_round33_pairresidual_softcap', 'summary': summary, 'rows': rows, 'extra': extra}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
