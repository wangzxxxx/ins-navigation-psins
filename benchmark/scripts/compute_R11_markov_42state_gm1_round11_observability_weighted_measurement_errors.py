...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round11_observability_weighted_measurement.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R11_markov_42state_gm1_round11_observability_weighted_measurement_param_errors.json'
...
meth = load(METHOD, 'meth_R11')
src = load(SRC, 'src_R11')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R11_markov_42state_gm1_round11_observability_weighted_measurement','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
