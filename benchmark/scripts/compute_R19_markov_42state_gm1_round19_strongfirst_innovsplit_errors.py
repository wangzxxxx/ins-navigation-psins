...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round19_strongfirst_innovsplit.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R19_markov_42state_gm1_round19_strongfirst_innovsplit_param_errors.json'
...
meth = load(METHOD, 'meth_R19')
src = load(SRC, 'src_R19')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R19_markov_42state_gm1_round19_strongfirst_innovsplit','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
