...
OUTPUT_JSON = RESULTS_DIR / 'round60_conservative_probe_summary.json'
CANDIDATE_JSON = RESULTS_DIR / 'round60_conservative_candidates.json'
...
ROUND60_CONSERVATIVE_CANDIDATES = [
    {
        'name': 'scale_once_a0999_s20tight_0899',
        'description': 'Keep Round59-H SCD unchanged and only tighten iter2 state20 by 0.001 to test a pure zz micro-repair.',
        'rationale': 'Most conservative follow-up after the softer-SCD probes failed: leave the SCD cut untouched and only nudge the dKg_zz state path.',
        'iter_patches': {
            1: {
                'state_alpha_mult': {20: 0.899},
            },
        },
    },
    {
        'name': 'scale_once_a0999_s20tight_0899_ryz00116',
        'description': 'Same exact R59-H SCD, same 0.001 state20 tighten, plus an ultra-small ry_z confirmation.',
        'rationale': 'Checks whether zz/ry_z can come back together with only the tiniest state20 + lever repair pair, while keeping the Round59-H main gain profile intact.',
        'iter_patches': {
            1: {
                'state_alpha_mult': {20: 0.899},
            },
        },
        'post_ry_z_mult': 1.00116,
    },
]
...
def _load_round59_base_candidate():
    return _merge_hybrid_candidate({
        ...
        'description': 'Round59-H formal best base candidate.',
        'rationale': 'Base for conservative Round60 refinements.',
        'scd': {
            'mode': 'once_per_phase',
            'alpha': 0.999,
...
def _run_candidate(candidate: dict, idx: int):
    ...
    method_mod = load_module(f'markov_method_round60_conservative_probe_{idx}', str(R53_METHOD_FILE))
    method_mod = _build_patched_method(method_mod, merged_candidate)
    method_mod.METHOD = f"42-state GM1 round60 conservative probe {merged_candidate['name']}"
    method_mod.VARIANT = f"42state_gm1_round60_conservative_probe_{merged_candidate['name']}"

    source_mod = load_module(f'markov_pruned_source_for_round60_conservative_probe_{idx}', str(SOURCE_FILE))
...
def main():
    ...
    candidate_dump = {
        ...
        'round60_conservative_candidates': ROUND60_CONSERVATIVE_CANDIDATES,
    }
    ...
    for idx, candidate in enumerate(ROUND60_CONSERVATIVE_CANDIDATES, start=1):
