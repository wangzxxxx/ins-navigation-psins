...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round12_aux_static_residual_twostage.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R12_markov_42state_gm1_round12_aux_static_residual_twostage_param_errors.json'
...
meth = load(METHOD, 'meth_R12')
src = load(SRC, 'src_R12')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R12_markov_42state_gm1_round12_aux_static_residual_twostage','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
