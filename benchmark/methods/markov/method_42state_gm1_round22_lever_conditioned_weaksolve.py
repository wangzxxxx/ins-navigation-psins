...
METHOD = '42-state GM1 round22 lever-conditioned single weak-state solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round22_lever_conditioned_weaksolve'
...
def _lever_conditioned_single_weaksolve(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    for s, e in windows:
        ...
        A_ka2 = mod.np.diag(mean_fb_sq)
        A_lev = mean_ss
        b = -vn / max(len(seg) * ts, 1e-12) - A_ka2 @ ka2_theta
        ...
        cond = smax / max(smin, 1e-12) if smax > 0 else 1e12
        cond_before.append(cond)

        quality = smin / max(smax, 1e-12)
        motion = mod.np.linalg.norm(mean_ss, ord='fro')
        weight = len(seg) * (0.20 + 2.50 * quality + 0.10 * motion)
        if quality < 0.16:
            continue
    ...
    if not lever_rows:
        return refined, {'n_windows': len(windows), 'used_windows': 0, 'cond_median': float(mod.np.median(cond_before)) if cond_before else None}
    ...
    col_scale = mod.np.maximum(col_scale, mod.np.array([0.10, 0.10, 0.10, 0.10, 0.10, 0.10]))
    As = Aw / col_scale[None, :]

    prior = -mod.np.concatenate((strong_clbt['rx'], strong_clbt['ry']))
    prior_s = prior * col_scale
    delta_sigma = mod.np.array([0.0025, 0.0020, 0.0020, 0.0025, 0.0020, 0.0020])
    ridge = mod.np.diag((col_scale / delta_sigma) ** 2)

    U, S, Vt = mod.np.linalg.svd(As, full_matrices=False)
    smax = float(S[0]) if len(S) else 0.0
    floor = max(0.22 * smax, 1e-9)
    Sinv = mod.np.array([s / (s * s + 0.18) if s >= floor else 0.0 for s in S])
    theta_ls = Vt.T @ (Sinv * (U.T @ bw)) if len(S) else prior_s.copy()

    lhs = As.T @ As + 0.10 * ridge
    rhs = As.T @ bw + 0.10 * ridge @ prior_s
    try:
    ...
    blend = 0.65
    theta_s = blend * theta_map + (1.0 - blend) * theta_ls
    ...
    delta = mod.np.clip(proposal - prior_lever, -0.0018, 0.0018)
...
def run_method():
    mod = load_module('markov_pruned_42_round22_lever_conditioned_weaksolve', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R22-BASE'
    )
    refined_clbt, extra = _lever_conditioned_single_weaksolve(mod, imu_noisy, pos0, ts, base_res[0])
