...
METHOD = '42-state GM1 round14 grouped conservative fusion'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round14_grouped_conservative_fusion'
...
def _blend_clbt(mod, base, proposal, alpha_main=0.6, alpha_weak=0.3, diag_anchor=0.1):
    out = _copy_clbt(mod, base)
    out['eb'] = (1 - alpha_main) * base['eb'] + alpha_main * proposal['eb']
    out['db'] = (1 - alpha_main) * base['db'] + alpha_main * proposal['db']
    out['Kg'] = (1 - alpha_main) * base['Kg'] + alpha_main * proposal['Kg']
    out['Ka'] = (1 - alpha_main) * base['Ka'] + alpha_main * proposal['Ka']
    out['Ka2'] = (1 - alpha_weak) * base['Ka2'] + alpha_weak * proposal['Ka2']
    out['rx'] = (1 - alpha_weak) * base['rx'] + alpha_weak * proposal['rx']
    out['ry'] = (1 - alpha_weak) * base['ry'] + alpha_weak * proposal['ry']
    for i in range(3):
        out['Kg'][i, i] = (1 - diag_anchor) * out['Kg'][i, i] + diag_anchor * base['Kg'][i, i]
        out['Ka'][i, i] = (1 - diag_anchor) * out['Ka'][i, i] + diag_anchor * base['Ka'][i, i]
    return out
...
def _aux_identify_weak_states(mod, imu1, pos0, ts, coarse_clbt):
    eth = mod.Earth(pos0)
    ...
    imu_corr = apply_strong_only(imu1, coarse_clbt)
    rows, ys = [], []
    for s, e in windows:
        seg = imu_corr[s:e, :]
        if len(seg) < 50:
    ...
    weak = _copy_clbt(mod, coarse_clbt)
    weak['Ka2'] = mod.np.clip(theta[0:3], -50 * mod.glv.ugpg2, 50 * mod.glv.ugpg2)
    lever = mod.np.clip(theta[3:9], -0.12, 0.12)
    weak['rx'] = lever[0:3]
    weak['ry'] = lever[3:6]
    refined['Ka2'] = 0.75 * coarse_clbt['Ka2'] + 0.25 * weak['Ka2']
    refined['rx'] = 0.7 * coarse_clbt['rx'] + 0.3 * weak['rx']
    refined['ry'] = 0.7 * coarse_clbt['ry'] + 0.3 * weak['ry']
    return refined


def _run_42_grouped(mod, imu1, pos0, ts, bi_g, bi_a, tau_g, tau_a, coarse_clbt, init_clbt, label):
    eth = mod.Earth(pos0)
    wnie = mod.glv.wie * mod.np.array([0, mod.math.cos(pos0[0]), mod.math.sin(pos0[0])])
    gn = mod.np.array([0, 0, -eth.g])
    Cba = mod.np.eye(3)
    ...
    alpha_main_seq = [0.45, 0.60]
    alpha_weak_seq = [0.18, 0.28]
    diag_anchor_seq = [0.35, 0.22]
    ...
    for it in range(iterations):
        ...
        if it == 0:
            kf['Pxk'][27:30, 27:30] *= 0.25
            kf['Pxk'][30:36, 30:36] *= 0.25
            kf['Pxk'][12:27, 12:27] *= 0.60
        elif it == 1:
            kf['Pxk'][27:30, 27:30] *= 0.40
            kf['Pxk'][30:36, 30:36] *= 0.40
            kf['Pxk'][12:27, 12:27] *= 0.75
        else:
            kf['Pxk'] = kf['Pxk'] * 100
            kf['Pxk'][:, 2] = 0
            kf['Pxk'][2, :] = 0
            kf['Pxk'][27:30, 27:30] *= 0.55
            kf['Pxk'][30:36, 30:36] *= 0.55
            kf['Pxk'][12:27, 12:27] *= 0.85
        ...
        if it != iterations - 1:
            ...
            clbt = _blend_clbt(
                mod,
                coarse_clbt if it == 0 else clbt,
                proposal,
                alpha_main=alpha_main_seq[it],
                alpha_weak=alpha_weak_seq[it],
                diag_anchor=diag_anchor_seq[it],
            )
...
def run_method():
    mod = load_module('markov_pruned_42_round14_grouped_conservative_fusion', str(TMP_PSINS / SOURCE))
    ...
    coarse_res = mod.run_calibration(
        ...
        label='42-GM1-R14-COARSE'
    )
    coarse_clbt = coarse_res[0]
    aux_clbt = _aux_identify_weak_states(mod, imu_noisy, pos0, ts, coarse_clbt)
    final_res = _run_42_grouped(
        mod, imu_noisy, pos0, ts, bi_g, bi_a, tau_g, tau_a,
        coarse_clbt=coarse_clbt, init_clbt=aux_clbt, label='42-GM1-R14-GROUPFUSE'
    )
    return final_res
