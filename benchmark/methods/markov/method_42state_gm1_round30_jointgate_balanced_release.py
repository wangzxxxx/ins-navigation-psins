...
METHOD = '42-state GM1 round30 joint-gated balanced weak release'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round30_jointgate_balanced_release'
...
def _jointgate_balanced_release_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    for s, e in windows:
        ...
        blocks.append((Awin, ywin, len(seg), cond, quality, innov, motion))
    ...
    rows, ys, weights = [], [], []
    trimmed = 0
    for Awin, ywin, seg_len, cond, quality, innov, motion in blocks:
    ...
    reg_sigma = mod.np.array([
        20.0 * mod.glv.ugpg2, 15.0 * mod.glv.ugpg2, 15.0 * mod.glv.ugpg2,
        0.00385, 0.0027, 0.0027,
        0.00385, 0.0027, 0.0027,
    ])
    ...
    raw_ka2_delta = mod.np.clip(proposal_ka2 - ka2_prior, -2.0 * mod.glv.ugpg2, 2.0 * mod.glv.ugpg2)
    ...
    lever_score = score[3:9]
    lever_diag_ratio = diag_ratio[3:9]
    lever_order = mod.np.argsort(lever_score)[::-1]
    ...
    lead_ratio = float(lever_score[lead] / max(lever_score[runner], 1e-18)) if lead is not None and runner is not None else None
    if lead is not None:
        trust[lead] = 0.438
    if runner is not None and lever_score[runner] / max(lever_score[lead], 1e-18) > 0.12:
        trust[runner] = min(0.118, 0.16 * lever_score[runner] / max(lever_score[lead], 1e-18))
    for idx in lever_order[2:]:
        if lever_diag_ratio[idx] < 0.16:
            continue
        if lever_score[idx] / max(lever_score[lead], 1e-18) < 0.045:
            continue
        trust[idx] = min(0.028, 0.05 * lever_score[idx] / max(lever_score[lead], 1e-18))
        break

    ka2_score = score[0:3]
    ka2_order = mod.np.argsort(ka2_score)[::-1]
    ka2_trust = mod.np.array([0.12, 0.08, 0.08])
    ka2_trust = ka2_trust[mod.np.argsort(ka2_order)]
    # reorder into axis order: strongest gets 0.12, others 0.08/0.08
    ka2_axis_trust = mod.np.zeros(3)
    if len(ka2_order) >= 1:
        ka2_axis_trust[ka2_order[0]] = 0.12
    if len(ka2_order) >= 2:
        ka2_axis_trust[ka2_order[1]] = 0.08
    if len(ka2_order) >= 3:
        ka2_axis_trust[ka2_order[2]] = 0.08

    lever_delta = trust * raw_lever_delta
    ka2_delta = ka2_axis_trust * raw_ka2_delta

    lever = mod.np.clip(lever_prior + lever_delta, -0.08, 0.08)
    ...
    return refined, {
        ...
        'lever_score': [float(x) for x in lever_score],
        'lever_diag_ratio': [float(x) for x in lever_diag_ratio],
        'lead_ratio': lead_ratio,
        'lever_trust': [float(x) for x in trust],
        'ka2_score_order': [int(i) for i in ka2_order],
        'ka2_score': [float(x) for x in ka2_score],
        'ka2_trust': [float(x) for x in ka2_axis_trust],
    }


def run_method():
    mod = load_module('markov_pruned_42_round30_jointgate_balanced_release', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R30-BASE'
    )
    refined_clbt, extra = _jointgate_balanced_release_refine(mod, imu_noisy, pos0, ts, base_res[0])
