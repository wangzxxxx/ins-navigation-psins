...
METHOD = '42-state GM1 round37 selective scale-block P6'
...
def run_method():
    mod = load_module('markov_pruned_42_round37_scaleblock_p6', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R37-BASE'
    )
    refined_clbt, xfb = _apply_selective_feedback(mod, base_res[0], base_res[1]['xk'])
    return refined_clbt, base_res[1], base_res[2], base_res[3], {
        ...
        'policy': 'Selective Kg/Ka main-block feedback only; eb/db/Ka2/rx/ry left unchanged.',
    }
