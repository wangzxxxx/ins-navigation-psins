...
METHOD = '42-state GM1 round23 soft-conditioned truncated-pinv lever solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round23_softcond_truncpinv'
...
def _soft_conditioned_trunc_pinv_weaksolve(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    if len(score):
        lead = int(mod.np.argmax(score))
        score_sorted = mod.np.sort(score)
        second = float(score_sorted[-2]) if len(score_sorted) >= 2 else 0.0
        dominance = float(score[lead] / max(second, 1e-18))
        if dominance >= 4.0:
            trust[lead] = min(0.5, 0.25 + 0.30 * float(diag_ratio[lead]))
        else:
            trust = mod.np.clip(0.10 + 0.40 * diag_ratio, 0.0, 0.35)
    ...
    return refined, {
        ...
        'dominant_index': int(mod.np.argmax(score)) if len(score) else None,
        'dominant_score_ratio': float(mod.np.max(score) / max(mod.np.partition(score, -2)[-2], 1e-18)) if len(score) >= 2 else None,
        'trust': [float(x) for x in trust],
    }


def run_method():
    mod = load_module('markov_pruned_42_round23_softcond_truncpinv', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R23-BASE'
    )
    refined_clbt, extra = _soft_conditioned_trunc_pinv_weaksolve(mod, imu_noisy, pos0, ts, base_res[0])
