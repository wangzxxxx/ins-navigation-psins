...
METHOD = '42-state GM1 round28 guarded balanced lever subblock finetune'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round28_subblock_guarded_balance'
...
def _guarded_balance_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    lead = int(order[0]) if len(order) else None
    ...
    if lead is not None:
        lead_sign = mod.np.sign(raw_delta[lead])
        for idx in order[1:]:
            if score[idx] < 0.04 * score[lead]:
                continue
            if diag_ratio[idx] < 0.78:
                continue
            if mod.np.sign(raw_delta[idx]) != lead_sign:
                continue
            trust[idx] = min(0.018, 0.060 * score[idx] / max(score[lead], 1e-18))
...
def run_method():
    mod = load_module('markov_pruned_42_round28_subblock_guarded_balance', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R28-BASE'
    )
    refined_clbt, extra = _guarded_balance_refine(mod, imu_noisy, pos0, ts, base_res[0])
