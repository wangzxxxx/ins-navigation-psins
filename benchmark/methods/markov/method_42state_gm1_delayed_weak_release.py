...
METHOD = '42-state GM1 delayed weak release'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_delayed_weak_release'


def run_method():
    mod = load_module('markov_pruned_42_delayed_release', str(TMP_PSINS / SOURCE))
    ts = 0.01
    ...
    # match fair baseline noisy settings
    ...
    # Inline copy of run_calibration with only one change:
    # delay weakly observable states Ka2/rx/ry feedback until final iteration.
    eth = mod.Earth(pos0)
    ...
    for it in range(iterations):
        print(f'  [42-GM1-DELAY] Iter {it+1}/{iterations}')
        ...
        if it != iterations - 1:
            # Delay Ka2/rx/ry release until final iteration; keep Kg/Ka/eb/db(+Markov bias) feedback.
            xk_saved = mod.np.copy(kf['xk'])
            kf['xk'][27:36] = 0.0
