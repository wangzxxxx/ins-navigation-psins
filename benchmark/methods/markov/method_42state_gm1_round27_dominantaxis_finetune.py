...
METHOD = '42-state GM1 round27 dominant-axis finetuned lever subblock solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round27_dominantaxis_finetune'
...
def _dominantaxis_finetune_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
...
def run_method():
    mod = load_module('markov_pruned_42_round27_dominantaxis_finetune', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R27-BASE'
    )
    refined_clbt, extra = _dominantaxis_finetune_refine(mod, imu_noisy, pos0, ts, base_res[0])
