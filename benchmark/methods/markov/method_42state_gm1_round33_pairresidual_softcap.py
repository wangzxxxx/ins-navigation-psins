...
METHOD = '42-state GM1 round33 pair-local residual split with cross-pair soft cap'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round33_pairresidual_softcap'
...
def _round33_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    prior_fit = As @ prior_s
    pair_base_residual = bw - prior_fit
    for i, j in pair_defs:
        ...
        pair_cols = As[:, [i, j]]
        pair_diag = mod.np.sum(pair_cols * pair_cols, axis=0) + 1e-18
        pair_reg = mod.np.diag(0.06 + 0.18 / mod.np.sqrt(pair_diag))
        pair_lhs = pair_cols.T @ pair_cols + pair_reg
        pair_rhs = pair_cols.T @ pair_base_residual
        try:
            pair_dz = mod.np.linalg.solve(pair_lhs, pair_rhs)
        except mod.np.linalg.LinAlgError:
            pair_dz, *_ = mod.np.linalg.lstsq(pair_lhs, pair_rhs, rcond=None)
        pair_local_delta = mod.np.clip(-(pair_dz / col_scale[[i, j]]), -0.0024, 0.0024)

        base_norm = float(mod.np.linalg.norm(pair_base_residual))
        resid_i = pair_base_residual - pair_cols[:, 0] * pair_dz[0]
        resid_j = pair_base_residual - pair_cols[:, 1] * pair_dz[1]
        drop_i = max(0.0, base_norm - float(mod.np.linalg.norm(resid_i)))
        drop_j = max(0.0, base_norm - float(mod.np.linalg.norm(resid_j)))
        amp_i = abs(float(pair_local_delta[0])) / (abs(float(raw_lever_delta[i])) + 5e-5)
        amp_j = abs(float(pair_local_delta[1])) / (abs(float(raw_lever_delta[j])) + 5e-5)
        evidence_i = drop_i * (0.80 + 0.20 * min(amp_i, 2.0))
        evidence_j = drop_j * (0.80 + 0.20 * min(amp_j, 2.0))
        evidence_sum = evidence_i + evidence_j
        if evidence_sum > 1e-15:
            balance = float(mod.np.clip(evidence_i / evidence_sum, 0.30, 0.70))
        else:
            balance = 0.5
        ...
        pair_balance.append({
            ...
            'local_residual_drop': [float(drop_i), float(drop_j)],
            'local_delta': [float(pair_local_delta[0]), float(pair_local_delta[1])],
            'split_basis': 'pair_local_residual_drop',
        })
...
def run_method():
    mod = load_module('markov_pruned_42_round33_pairresidual_softcap', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R33-BASE'
    )
    refined_clbt, extra = _round33_refine(mod, imu_noisy, pos0, ts, base_res[0])
