...
METHOD = '42-state GM1 round26 pair-gated micro-tail lever subblock solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round26_pairgate_microtail'
...
def _pairgated_microtail_lever_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
...
def run_method():
    mod = load_module('markov_pruned_42_round26_pairgate_microtail', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R26-BASE'
    )
    refined_clbt, extra = _pairgated_microtail_lever_refine(mod, imu_noisy, pos0, ts, base_res[0])
