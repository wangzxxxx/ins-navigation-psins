...
METHOD = '42-state GM1 staged QR schedule'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_staged_qr_schedule'


def run_method():
    mod = load_module('markov_pruned_42_staged_qr', str(TMP_PSINS / SOURCE))
    ts = 0.01
    ...
    def apply_schedule(kf, phase_mode):
        # phase_mode: 'static' or 'dynamic'
        if phase_mode == 'static':
            # During static segments, suppress weak-state drift and slightly trust measurements less.
            kf['Qk'][27:36, 27:36] *= 0.2
            kf['Rk'] *= 1.15
        else:
            # During rotating/dynamic segments, keep weak-state adaptation alive.
            kf['Qk'][27:36, 27:36] *= 1.5
        return kf

    for it in range(iterations):
        print(f'  [42-GM1-STAGED-QR] Iter {it+1}/{iterations}')
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s > (0.2 - ts / 2):
                ...
                kf = apply_schedule(kf, 'static' if static_phase else 'dynamic')
                if static_phase:
