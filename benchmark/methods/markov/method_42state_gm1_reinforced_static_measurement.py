...
METHOD = '42-state GM1 reinforced static measurement'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_reinforced_static_measurement'


def run_method():
    mod = load_module('markov_pruned_42_reinforced_static_measurement', str(TMP_PSINS / SOURCE))
    ts = 0.01
    ...
    for it in range(iterations):
        print(f'  [42-GM1-REINFORCED-M] Iter {it+1}/{iterations}')
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if mod.np.linalg.norm(ww) / ts < 20 * mod.glv.dph:
                    ...
                    # reinforced but mild second pass with half innovation to better absorb static observability
                    kf = mod.kfupdate(kf, yk=0.5 * vn, TimeMeasBoth='M')
