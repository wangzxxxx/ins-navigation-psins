...
METHOD = '42-state GM1 round9 adaptive soft anchor'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round9_adaptive_soft_anchor'
...
def _shape_target_block(mod, kf, stage: int):
    # Compared with round8: keep the same 19-position path/time, but slightly reduce
    # early over-amplification and add a little more Ka2<->lever-arm support.
    if stage == 0:
        gain = 1.28
        corr = 0.12
        q_gain = 1.12
    ...
    for ka2_idx in TARGET_KA2_IDXS:
        for lr_idx in TARGET_RXRY_IDXS:
            sym = ka2_lr_corr * (kf['Pxk'][ka2_idx, ka2_idx] * kf['Pxk'][lr_idx, lr_idx]) ** 0.5
            kf['Pxk'][ka2_idx, lr_idx] = sym
            kf['Pxk'][lr_idx, ka2_idx] = sym


def run_method():
    mod = load_module('markov_pruned_42_round9_adaptive_soft_anchor', str(TMP_PSINS / SOURCE))
    ...
    for it in range(iterations):
        print(f'  [42-GM1-R9-ADAPTIVE-SOFT-ANCHOR] Iter {it+1}/{iterations}')
        kf = mod.clbtkfinit_42(nts, bi_g, tau_g, bi_a, tau_a)
        _shape_target_block(mod, kf, it)
        if it == iterations - 1:
            kf['Pxk'] = kf['Pxk'] * 60
            kf['Pxk'][:, 2] = 0
            kf['Pxk'][2, :] = 0
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            # Adaptive but still full-state: lighter dKg_xx forcing, stronger Ka2/lever-arm coupling.
            Ft[27:30, TARGET_DKGXX_IDX] *= 1.06
            Ft[30:36, TARGET_DKGXX_IDX] *= 1.04
            Ft[27:30, 30:36] *= 1.05
            Ft[30:36, 27:30] *= 1.05
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if mod.np.linalg.norm(ww) / ts < 20 * mod.glv.dph:
                    ...
                    if it == 0:
                        ...
                        yk *= 0.99
                    kf = mod.kfupdate(kf, yk=yk, TimeMeasBoth='M')
