...
import psins_py.test_calibration_schmidt_llm as src

SOURCE = 'test_calibration_schmidt_llm.py'
METHOD = 'Noisy w/ LLM Masking (Scheme A)'
...
def run_method():
    ...
    res = src.run_calibration(imu_noisy, pos0, ts, llm_mode=True, label='Noisy LLM')
    metrics = extract_result_metrics('llm_masking_scheme_a', res)
    metrics.update({
        'source': SOURCE,
        'method_label': METHOD,
