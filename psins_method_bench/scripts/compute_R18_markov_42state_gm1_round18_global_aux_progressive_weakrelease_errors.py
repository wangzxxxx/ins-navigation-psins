...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round18_global_aux_progressive_weakrelease.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R18_markov_42state_gm1_round18_global_aux_progressive_weakrelease_param_errors.json'
...
meth = load(METHOD, 'meth_R18')
src = load(SRC, 'src_R18')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R18_markov_42state_gm1_round18_global_aux_progressive_weakrelease','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
