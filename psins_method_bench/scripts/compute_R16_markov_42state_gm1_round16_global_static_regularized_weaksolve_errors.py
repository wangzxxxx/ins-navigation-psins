...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round16_global_static_regularized_weaksolve.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R16_markov_42state_gm1_round16_global_static_regularized_weaksolve_param_errors.json'
...
meth = load(METHOD, 'meth_R16')
src = load(SRC, 'src_R16')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R16_markov_42state_gm1_round16_global_static_regularized_weaksolve','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
