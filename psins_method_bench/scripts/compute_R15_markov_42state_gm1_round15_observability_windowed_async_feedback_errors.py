...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round15_observability_windowed_async_feedback.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R15_markov_42state_gm1_round15_observability_windowed_async_feedback_param_errors.json'
...
meth = load(METHOD, 'meth_R15')
src = load(SRC, 'src_R15')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R15_markov_42state_gm1_round15_observability_windowed_async_feedback','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
