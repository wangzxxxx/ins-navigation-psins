...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round28_subblock_guarded_balance.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R28_markov_42state_gm1_round28_subblock_guarded_balance_param_errors.json'
...
meth = load(METHOD, 'meth_R28')
src = load(SRC, 'src_R28')
res = meth.run_method()
clbt = res[0]
extra = res[4]
...
OUT.write_text(json.dumps({'method': 'R28_markov_42state_gm1_round28_subblock_guarded_balance', 'summary': summary, 'rows': rows, 'extra': extra}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
