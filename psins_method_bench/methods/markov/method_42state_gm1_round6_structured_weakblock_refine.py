...
METHOD = '42-state GM1 round6 structured weak-block refine'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round6_structured_weakblock_refine'


STRONG_IDX = list(range(6, 27)) + list(range(36, 42))   # eb,db,dKg,dKa,bm_g,bm_a
WEAK_IDX = list(range(27, 36))                           # Ka2,rx,ry
TARGETED_KG_DIAG_IDX = [12, 16, 20]                      # dKg_xx,dKg_yy,dKg_zz inside xk
...
def _apply_weak_feedback(mod, clbt, xk, n_states=42, kg_diag_gain=0.35):
    subkf = {'xk': mod.np.zeros_like(xk)}
    subkf['xk'][27:36] = xk[27:36]
    # targeted weak companion update: only allow gyro scale diagonal to move a little,
    # giving Ka2/lever-arm refinement a minimal structured anchor without re-releasing all dKg.
...
def run_method():
    mod = load_module('markov_pruned_42_round6_structured_weakblock_refine', str(TMP_PSINS / SOURCE))
    ...
    def run_single_pass(label, freeze_strong=False, inflate_weak_p=False):
        print(label)
        kf = mod.clbtkfinit_42(nts, bi_g, tau_g, bi_a, tau_a)
        if inflate_weak_p:
            kf['Pxk'][27:36, 27:36] *= 16.0
            for idx in TARGETED_KG_DIAG_IDX:
                kf['Pxk'][idx, idx] *= 4.0
    ...
    # Stage 1: let strong states settle in two standard passes, but do not release weak block yet.
    for it in range(2):
        kf, local_P, local_X = run_single_pass(f'  [42-GM1-R6-STRUCT] strong pass {it+1}/2')
    ...
    # Stage 2: weak-block refinement with strong states frozen; only weak block + tiny dKg diagonal companion released.
    kf, local_P, local_X = run_single_pass('  [42-GM1-R6-STRUCT] weak-block refine', freeze_strong=True, inflate_weak_p=True)
    clbt = _apply_weak_feedback(mod, clbt, kf['xk'], 42, kg_diag_gain=0.35)
    P_trace.extend(local_P)
    X_trace.extend(local_X)
    iter_bounds.append(len(P_trace))
