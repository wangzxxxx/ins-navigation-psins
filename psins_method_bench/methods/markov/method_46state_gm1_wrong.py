...
SOURCE = 'test_calibration_markov_noise_46state.py'
METHOD = '46-state GM1 wrong tau'
FAMILY = 'triangular_ka_markov'
VARIANT = '46state_gm1_wrong_tau'


def run_method():
    mod = load_module('markov_46_wrong', str(TMP_PSINS / SOURCE))
    ts = 0.01
    ...
    imu_noisy = mod.imuadderr_full(
        imu_clean, ts,
        arw=0.01 * mod.glv.dpsh, vrw=10.0 * mod.glv.ugpsHz,
        bi_g=0.005 * mod.glv.dph, tau_g=300.0,
        bi_a=10.0 * mod.glv.ug, tau_a=300.0, seed=42,
    )
    return mod.run_calibration_46(
        imu_noisy, pos0, ts,
        bi_g=0.005 * mod.glv.dph, tau_g=3600.0,
        bi_a=10.0 * mod.glv.ug, tau_a=3600.0,
