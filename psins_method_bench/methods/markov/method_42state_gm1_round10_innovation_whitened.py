...
METHOD = '42-state GM1 round10 innovation-whitened measurement'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round10_innovation_whitened'
...
def run_method():
    mod = load_module('markov_pruned_42_round10_innovation_whitened', str(TMP_PSINS / SOURCE))
    ...
    for it in range(iterations):
        print(f'  [42-GM1-R10-INNOV-WHITEN] Iter {it+1}/{iterations}')
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if mod.np.linalg.norm(ww) / ts < 20 * mod.glv.dph:
                    ...
                    pred = kf['Hk'] @ kf['xk']
                    innov = yk - pred
                    pred_cov = kf['Hk'] @ kf['Pxk'] @ kf['Hk'].T + mod.np.diag(base_r_diag)
                    try:
                        nis = float(innov.T @ mod.np.linalg.inv(pred_cov) @ innov)
                    except mod.np.linalg.LinAlgError:
                        nis = float(innov.T @ mod.np.linalg.pinv(pred_cov) @ innov)
                    nis_ema = 0.90 * nis_ema + 0.10 * (nis / 3.0)

                    # Innovation normalization instead of state anchoring:
                    # high-energy static residuals get temporarily down-weighted;
                    # cleaner residuals later get relatively stronger updates.
                    scale = float(mod.np.clip(nis_ema, 0.55, 2.60))
                    if it == 0:
                        scale *= 1.10
                    elif it == 2:
                        scale *= 0.92
                    scale = float(mod.np.clip(scale, 0.50, 2.80))
                    kf['Rk'] = mod.np.diag(base_r_diag * scale)
