...
METHOD = '42-state GM1 round31 decoupled anisotropic Ka2 bypass with gated lever trust'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round31_decoupled_bypass_anisoweak'
...
def _round31_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    for Dwin, SSwin, ywin, seg_len, cond, quality, innov, motion in blocks:
        ...
        ka2_rhs_rows.append(ywin.copy())

    # lever solve: keep R30 dominant + secondary trust, but solve lever alone.
    ...
    lever_delta = lever_trust * raw_lever_delta
    lever = mod.np.clip(lever_prior + lever_delta, -0.08, 0.08)
    refined['rx'] = lever[0:3]
    refined['ry'] = lever[3:6]

    # Ka2 bypass solve: use residual after accepted lever update, then solve each axis independently.
    lever_effect_rows = []
    ...
    return refined, {
        ...
        'lever_trust': [float(x) for x in lever_trust],
...
def run_method():
    mod = load_module('markov_pruned_42_round31_decoupled_bypass_anisoweak', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R31-BASE'
    )
    refined_clbt, extra = _round31_refine(mod, imu_noisy, pos0, ts, base_res[0])
