...
METHOD = '42-state GM1 round12 auxiliary static-residual two-stage'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round12_aux_static_residual_twostage'
...
def _run_42_with_init(mod, imu1, pos0, ts, bi_g, bi_a, tau_g, tau_a, init_clbt, label):
    eth = mod.Earth(pos0)
    ...
    for it in range(iterations):
        ...
        # Stage-2 formal run: retain the auxiliary weak-state estimates and only let KF refine around them.
        if it == 0:
            kf['Pxk'][27:30, 27:30] *= 0.35
            kf['Pxk'][30:36, 30:36] *= 0.35
        elif it == 1:
            ...
            kf['Pxk'][:, 2] = 0
            kf['Pxk'][2, :] = 0
            kf['Pxk'][27:30, 27:30] *= 0.70
            kf['Pxk'][30:36, 30:36] *= 0.70
...
def _aux_identify_weak_states(mod, imu1, pos0, ts, coarse_clbt):
    ...
    if not windows:
        return _copy_clbt(mod, coarse_clbt)
    ...
    for s, e in windows:
        ...
        for k in range(len(seg)):
            ...
            feat = mod.np.hstack((mod.np.tile((fb**2).reshape(3, 1), (1, 3)), SS[:, 0:6]))
            rows.append(feat)
        ...
        # Auxiliary quantity: per-static-window velocity-closure residual.
        if weight > 0:
            mean_fb = mod.np.mean(seg[:, 3:6] / ts, axis=0)
            mean_feat = mod.np.hstack((mod.np.tile((mean_fb**2).reshape(3, 1), (1, 3)), (ss_sum / weight)))
            rows.append(mean_feat)
            ys.append(-vn / max(weight * ts, 1e-12))
    ...
    A_blocks = []
    b_blocks = []
    for feat, y in zip(rows, ys):
        # fn = qnb*(fb - Ka2*fb^2 - SS*[rx;ry]); after moving terms: y ≈ qnb*(Ka2*fb^2 + SS*[rx;ry])
        # Here qnb is already absorbed by using navigation-frame closure target from same segment.
        A_blocks.append(feat)
        b_blocks.append(y)
    A = mod.np.vstack(A_blocks)
    b = mod.np.hstack(b_blocks)
    try:
    ...
    refined['Ka2'] = mod.np.clip(theta[0:3], -60 * mod.glv.ugpg2, 60 * mod.glv.ugpg2)
    lever = mod.np.clip(theta[3:9], -0.20, 0.20)
...
def run_method():
    mod = load_module('markov_pruned_42_round12_aux_static_residual_twostage', str(TMP_PSINS / SOURCE))
