...
METHOD = '42-state GM1 round19 strong-first innovation split'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round19_strongfirst_innovsplit'
...
def _global_aux_weaksolve(mod, imu1, pos0, ts, strong_clbt):
    ...
    refined['Ka2'] = mod.np.clip(-theta[0:3], -24 * mod.glv.ugpg2, 24 * mod.glv.ugpg2)
    lever = mod.np.clip(-theta[3:9], -0.055, 0.055)
    refined['rx'] = lever[0:3]
    refined['ry'] = lever[3:6]
    return refined


def _split_measurement_update(mod, kf, yk, strong_idx, weak_idx):
    H = kf['Hk']
    R = kf['Rk']
    P = kf['Pxk']
    x = kf['xk']

    def _masked_update(x_in, P_in, idx, resid, gain_scale):
        Pxy = P_in @ H.T
        S = H @ Pxy + R
        try:
            S_inv = mod.np.linalg.inv(S)
        except mod.np.linalg.LinAlgError:
            S_inv = mod.np.linalg.pinv(S)
        K = Pxy @ S_inv
        mask = mod.np.zeros(K.shape[0])
        mask[idx] = 1.0
        K = gain_scale * (K * mask[:, None])
        I = mod.np.eye(P_in.shape[0])
        IKH = I - K @ H
        x_out = x_in + K @ resid
        P_out = IKH @ P_in @ IKH.T + K @ R @ K.T
        P_out = (P_out + P_out.T) * 0.5
        P_out = mod.np.nan_to_num(P_out, nan=1e6, posinf=1e6, neginf=-1e6)
        resid_out = yk - H @ x_out
        return x_out, P_out, resid_out
 
     resid0 = yk - H @ x
-    x, P, resid1 = _masked_update(x, P, strong_idx, resid0)
-    x, P, resid2 = _masked_update(x, P, weak_idx, resid1)
+    x, P, resid1 = _masked_update(x, P, strong_idx, resid0, 0.92)
+    x, P, resid2 = _masked_update(x, P, weak_idx, resid1, 0.55)
...
def _run_strongfirst_innovsplit(mod, imu1, pos0, ts, bi_g, bi_a, tau_g, tau_a, init_clbt, label):
    eth = mod.Earth(pos0)
    wnie = mod.glv.wie * mod.np.array([0, mod.math.cos(pos0[0]), mod.math.sin(pos0[0])])
    gn = mod.np.array([0, 0, -eth.g])
    Cba = mod.np.eye(3)
    ...
    strong_idx = list(range(0, 27)) + list(range(36, 42))
    ...
    for it in range(3):
        ...
        for k in range(2 * frq2, len(imu1) - frq2, nn):
            ...
            if t1s > (0.2 - ts / 2):
                ...
                ww = mod.np.mean(imu1[k-frq2:k+frq2+1, 0:3], axis=0)
                if mod.np.linalg.norm(ww) / ts < 20 * mod.glv.dph:
                    kf = _split_measurement_update(mod, kf, vn, strong_idx, weak_idx)
...
def run_method():
    mod = load_module('markov_pruned_42_round19_strongfirst_innovsplit', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R19-BASE'
    )
    aux_clbt = _global_aux_weaksolve(mod, imu_noisy, pos0, ts, base_res[0])
    init_clbt = _copy_clbt(mod, base_res[0])
    init_clbt['Ka2'] = 0.75 * base_res[0]['Ka2'] + 0.25 * aux_clbt['Ka2']
    init_clbt['rx'] = 0.75 * base_res[0]['rx'] + 0.25 * aux_clbt['rx']
    init_clbt['ry'] = 0.75 * base_res[0]['ry'] + 0.25 * aux_clbt['ry']
    return _run_strongfirst_innovsplit(
        mod, imu_noisy, pos0, ts, bi_g, bi_a, tau_g, tau_a,
        init_clbt=init_clbt, label='42-GM1-R19-STRONGFIRST'
    )
