...
METHOD = '42-state GM1 coupled weak-state assist'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_coupled_weakstate_assist'


def run_method():
    mod = load_module('markov_pruned_42_coupled_assist', str(TMP_PSINS / SOURCE))
    ts = 0.01
    ...
    for it in range(iterations):
        print(f'  [42-GM1-COUPLED-ASSIST] Iter {it+1}/{iterations}')
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            # Coupled assist: slightly strengthen coupling from gyro/accel scale-factor block to weak states.
            Ft[27:30, 12:21] *= 1.25   # Ka2 gets a bit more sensitivity to scale/misalignment states
            Ft[30:36, 12:21] *= 1.15   # lever-arm-like states get a mild extra coupling
