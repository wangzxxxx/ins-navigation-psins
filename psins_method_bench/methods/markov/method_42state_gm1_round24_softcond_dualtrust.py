...
METHOD = '42-state GM1 round24 soft-conditioned trust-saturated truncated-pinv lever solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round24_softcond_dualtrust'
...
def _soft_conditioned_trunc_pinv_trustsat(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    delta_sigma = mod.np.array([0.0040, 0.0030, 0.0030, 0.0040, 0.0030, 0.0030])
    ...
    second_ratio = None
    if len(score):
        order = mod.np.argsort(score)
        lead = int(order[-1])
        second_idx = int(order[-2]) if len(order) >= 2 else None
        second = float(score[second_idx]) if second_idx is not None else 0.0
        dominance = float(score[lead] / max(second, 1e-18))
        if second_idx is not None:
            second_ratio = float(score[second_idx] / max(score[lead], 1e-18))
        if dominance >= 4.0:
            lead_trust = min(0.47, 0.22 + 0.30 * float(diag_ratio[lead]))
            trust[lead] = lead_trust
        else:
            trust = mod.np.clip(0.08 + 0.32 * diag_ratio * mod.np.sqrt(score / max(score[lead], 1e-18)), 0.0, 0.24)
    ...
    return refined, {
        ...
        'dominant_index': lead,
        'runnerup_index': second_idx,
        'dominant_score_ratio': dominance,
        'runnerup_score_ratio': second_ratio,
        'trust': [float(x) for x in trust],
    }


def run_method():
    mod = load_module('markov_pruned_42_round24_softcond_dualtrust', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R24-BASE'
    )
    refined_clbt, extra = _soft_conditioned_trunc_pinv_trustsat(mod, imu_noisy, pos0, ts, base_res[0])
