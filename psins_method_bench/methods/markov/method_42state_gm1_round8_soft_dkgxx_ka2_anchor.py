...
METHOD = '42-state GM1 round8 soft dKg_xx Ka2 anchor'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round8_soft_dkgxx_ka2_anchor'

TARGET_DKGXX_IDX = 12
TARGET_KA2_IDXS = [27, 28, 29]
TARGET_RXRY_IDXS = list(range(30, 36))
TARGET_BLOCK = [TARGET_DKGXX_IDX] + TARGET_KA2_IDXS + TARGET_RXRY_IDXS
...
def _soft_shape_target_block(mod, kf, stage: int):
    # Mildly enlarge uncertainty for dKg_xx / Ka2 / rx-ry so they remain learnable,
    # while adding light local correlation instead of hard freezing or isolated passes.
    if stage == 0:
        gain = 1.35
        corr = 0.10
        q_gain = 1.15
    ...
    for idx in TARGET_BLOCK:
        kf['Pxk'][idx, idx] *= gain
        kf['Qk'][idx, idx] *= q_gain

    # Encourage Ka2 to keep weak linkage with dKg_xx and lever-arm states instead of decoupling them.
    for idx in TARGET_KA2_IDXS + TARGET_RXRY_IDXS:
        sym = corr * (kf['Pxk'][TARGET_DKGXX_IDX, TARGET_DKGXX_IDX] * kf['Pxk'][idx, idx]) ** 0.5
        kf['Pxk'][TARGET_DKGXX_IDX, idx] = sym
        kf['Pxk'][idx, TARGET_DKGXX_IDX] = sym

    # Add very light Ka2 <-> rx/ry cross support.
    for ka2_idx in TARGET_KA2_IDXS:
        for lr_idx in TARGET_RXRY_IDXS:
            sym = 0.035 * (kf['Pxk'][ka2_idx, ka2_idx] * kf['Pxk'][lr_idx, lr_idx]) ** 0.5
            kf['Pxk'][ka2_idx, lr_idx] = sym
            kf['Pxk'][lr_idx, ka2_idx] = sym


def run_method():
    mod = load_module('markov_pruned_42_round8_soft_dkgxx_ka2_anchor', str(TMP_PSINS / SOURCE))
    ...
    for it in range(iterations):
        print(f'  [42-GM1-R8-SOFT-ANCHOR] Iter {it+1}/{iterations}')
        kf = mod.clbtkfinit_42(nts, bi_g, tau_g, bi_a, tau_a)
        _soft_shape_target_block(mod, kf, it)
        if it == iterations - 1:
            kf['Pxk'] = kf['Pxk'] * 100
            kf['Pxk'][:, 2] = 0
            kf['Pxk'][2, :] = 0
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            # Soft structural assist only: preserve full-state evolution, but slightly emphasize
            # the physically relevant dKg_xx -> Ka2/rx/ry interaction channels.
            Ft[27:30, TARGET_DKGXX_IDX] *= 1.08
            Ft[30:36, TARGET_DKGXX_IDX] *= 1.05
            Ft[27:30, 30:36] *= 1.03
            Ft[30:36, 27:30] *= 1.03
