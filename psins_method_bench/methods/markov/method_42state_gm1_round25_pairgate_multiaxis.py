...
METHOD = '42-state GM1 round25 pair-gated multi-axis lever subblock solve'
FAMILY = 'pruned_markov'
VARIANT = '42state_gm1_round25_pairgate_multiaxis'
...
def _pairgated_multiaxis_lever_refine(mod, imu1, pos0, ts, strong_clbt):
    eth = mod.Earth(pos0)
    ...
    raw_delta = mod.np.clip(proposal - prior_lever, -0.0022, 0.0022)
    ...
    if len(order) >= 2 and score[order[1]] / max(score[order[0]], 1e-18) > 0.15:
        trust[order[1]] = 0.14
    if len(order) >= 3 and diag_ratio[order[2]] > 0.12:
        trust[order[2]] = 0.06
...
def run_method():
    mod = load_module('markov_pruned_42_round25_pairgate_multiaxis', str(TMP_PSINS / SOURCE))
    ...
    base_res = mod.run_calibration(
        ...
        label='42-GM1-R25-BASE'
    )
    refined_clbt, extra = _pairgated_multiaxis_lever_refine(mod, imu_noisy, pos0, ts, base_res[0])
