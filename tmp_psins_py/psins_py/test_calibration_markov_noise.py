...
def main():
    ...
    # 噪声参数（降低到典型值，观察标定效果）
    ARW   = 0.001  * glv.dpsh       # 角度随机游走: 0.01 deg/sqrt(h)
    VRW   = 1.0  * glv.ugpsHz     # 速度随机游走: 10 ug/sqrt(Hz)
    BI_G  = 0.005 * glv.dph        # 陀螺零偏不稳定性: 0.005 deg/h (典型值)
    BI_A  = 10.0  * glv.ug         # 加计零偏不稳定性: 10 ug       (典型值)
    TAU_G = 3000.0                   # 相关时间: 300s
    TAU_A = 3000.0                   # 相关时间: 300s
    ...
    # ── B: 全噪声 + 43 状态（不建模 Markov）──
    print("\n[B] Full noise, 43-state (no Markov model)...")
    clbt_B = run_calibration(imu_noisy, pos0, ts, n_states=43, label="43-state")

    # ── C: 全噪声 + 49 状态（正确 Markov 参数）──
    print("\n[C] Full noise, 49-state (correct tau/sigma)...")
    clbt_C = run_calibration(imu_noisy, pos0, ts, n_states=49,
                              bi_g=BI_G, tau_g=TAU_G, bi_a=BI_A, tau_a=TAU_A,
                              label="49-correct")

    # ── D: 全噪声 + 49 状态（错误的 tau：真实 300s，设成 3600s）──
    TAU_WRONG = 3600.0  # 故意设错 12 倍
    print(f"\n[D] Full noise, 49-state (WRONG tau={TAU_WRONG}s, truth={TAU_G}s)...")
    clbt_D = run_calibration(imu_noisy, pos0, ts, n_states=49,
                              bi_g=BI_G, tau_g=TAU_WRONG, bi_a=BI_A, tau_a=TAU_WRONG,
                              label="49-wrong-tau")

    # ── E: 全噪声 + 55 状态（二阶 Gauss-Markov，正确 tau）──
    print("\n[E] Full noise, 55-state (2nd-order GM, correct tau)...")
    clbt_E = run_calibration(imu_noisy, pos0, ts, n_states=55,
                              bi_g=BI_G, tau_g=TAU_G, bi_a=BI_A, tau_a=TAU_A,
                              label="55-SOGM")
