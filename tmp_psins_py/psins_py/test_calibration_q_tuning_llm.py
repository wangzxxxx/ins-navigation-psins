...
def get_llm_q_scalar(history_context, kf_state, kf_diag_p):
    """
    LLM evaluates the stability of scale factors and misalignments during stationary periods
    and outputs a Process Noise scalar (Q_SCALAR) to control the learning rate of unobservable parameters.
    """
    global is_first_llm_call
    if client is None:
        return 1.0
    
    # Extract states 12-35
    state_info_lines = []
    ...
    system_rules = """
    You are an autonomous expert Kalman Filter Process Noise (Q-Matrix) scheduler.
    Indices 12-35 are scale factor and misalignment parameters.
    
    CRITICAL CONCEPT: When the turntable is STRICTLY STATIONARY, rotation-dependent scale factors and misalignments (Indices 12-35) do NOT physically change. 
    However, standard Kalman Filters assume they drift over time due to Process Noise (Q-matrix), making the filter overly eager to update them with measurement noise when it shouldn't.

    *YOUR AUTONOMOUS TASK*:
    You will be given the CURRENT ACTUAL NUMERICAL STATE of the filter (Value and StdDev for indices 12-35) and motion history.
    Based on this, tell the filter how much to shrink the PROCESS NOISE for these scale factor parameters (Indices 12-35) by providing a single multiplier `Q_SCALAR` (a float between 0.001 and 1.0).
    - If stationary and states are drifting randomly, output a TINY value like 0.01 or 0.1 to lock down their physical drift assumption.
    - If rotating or recovering, output 1.0.
    
    Format:
    REASONING: The turntable is stationary. Scale factors should maintain their physical values. Shrinking Q to prevent noise drift.
    Q_SCALAR: 0.05
    """
...
{state_info_str}
    ...
    Tell me the Q_SCALAR to apply to indices 12-35 AT THIS VERY MOMENT.
    (Output your response in the requested REASONING and Q_SCALAR format).
    """

    if is_first_llm_call:
        messages = [
            {"role": "system", "content": "You are a precise Kalman filter Q-matrix scheduler."},
    ...
    try:
        ...
        match = re.search(r'Q_SCALAR:\s*([0-9.]+)', content)
        if match:
            scalar = float(match.group(1))
            return min(max(scalar, 0.001), 1.0) # Bound between 0.001 and 1.0
        return 1.0
    except Exception as e:
        print(f"    [LLM] Error: {e}")
        return 1.0
...
def run_calibration(imu1, pos0, ts, llm_mode=False, label=""):
    ...
    for it in range(iterations):
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if current_motion_status == "STATIONARY":
                ...
                if stationary_time > 5.0 and llm_mode and not llm_called_this_phase:
                    if it == 0:
                        ...
                        current_q_scalar = get_llm_q_scalar(recent_history, kf['xk'], np.diag(kf['Pxk']))
                        state_key = "_".join([h.split("->")[1].strip() for h in motion_history[-4:]])
                        global_llm_mask_cache[state_key] = current_q_scalar
                    else:
                        state_key = "_".join([h.split("->")[1].strip() for h in motion_history[-4:]])
                        current_q_scalar = global_llm_mask_cache.get(state_key, 1.0)
            ...
            if llm_mode and current_q_scalar < 1.0:
                original_Qk = np.copy(kf['Qk'])
                # Scale process noise variance explicitly for indices 12-35
                for idx in range(12, 36):
                    kf['Qk'][idx, idx] *= current_q_scalar
                kf = kfupdate(kf, TimeMeasBoth='T')
                kf['Qk'] = original_Qk
                
                # Apply Covariance Fading (Exponential P_k Decay) to unobservable states
                # S is a diagonal scaling matrix. P = S * P * S
                decay_rate = 0.005 # 0.5% decay per 100Hz step when Q is suppressed
                fade_factor = 1.0 - (1.0 - current_q_scalar) * decay_rate
                fade_scalar = math.sqrt(max(fade_factor, 0.9)) # Safety bound on decay
                
                S = np.ones(36)
                S[12:36] = fade_scalar
                S_mat = np.diag(S)
                kf['Pxk'] = S_mat @ kf['Pxk'] @ S_mat
                
                q_scalar_applied_count += 1
            else:
                kf = kfupdate(kf, TimeMeasBoth='T')
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if np.linalg.norm(ww) / ts < 20 * glv.dph:
                    # Standard, unspoiled zero bias measurement update
        ...
        if llm_mode:
            print(f"    [Debug] Over this iteration, Q-matrix tuning was applied {q_scalar_applied_count} times.")
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plots_q_tuning_llm')
