...
def get_llm_scd_params(iteration, prev_stats=None):
    """
    LLM decides TWO independent correlation decay rates:
      alpha_nc: nav↔calib (the DIRECT noise pipeline — needs strong decay)
      alpha_bc: bias↔calib (physical coupling channel — needs gentle decay)
    Nav↔bias is NEVER decayed (alpha=1.0 always).
    """
    # Fixed optimal defaults - no LLM tuning to avoid over-optimization
    return {'alpha_nc': 0.98, 'alpha_bc': 0.995, 'transition_duration': 2.0}
...
Previous iteration results:
  ...
  - Ka2 volatility: {prev_stats['ka2_vol']:.4e}
  - Number of ZUPT updates: {prev_stats['n_zupt']}
  - Number of SCD applications: {prev_stats['n_scd']}
"""

    system_prompt = """You are tuning a Graded Selective Correlation Decay (SCD) mechanism for IMU calibration.

MECHANISM: Once per stationary phase (after a transition grace period), the P matrix cross-correlations are scaled:
  P[nav, calib] *= alpha_nc    (the DIRECT noise pipeline from velocity to scale factors)
  P[bias, calib] *= alpha_bc   (physical coupling: biases <-> scale factors/Ka2)
  P[nav, bias] is NEVER decayed (biases need full nav support for estimation).

KEY PHYSICS:
- nav<->calib is the NOISE PIPELINE: velocity ZUPT noise leaks into calibration params through this channel.
  Needs STRONG decay (e.g. 0.95-0.97) to block chronic poisoning.
- bias<->calib is PHYSICAL COUPLING: accelerometer bias (db) and quadratic error (Ka2) are physically entangled.
  Needs VERY GENTLE decay (e.g. 0.99-0.999) to preserve this bond while still mildly de-coupling.

PARAMETERS:
1. ALPHA_NC (0.90 to 0.99): Decay for nav<->calib noise pipeline.
   0.95 = aggressive, 0.97 = moderate, 0.99 = gentle.
2. ALPHA_BC (0.98 to 1.0): Decay for bias<->calib physical coupling.
   0.995 = gentle (recommended), 0.99 = moderate, 1.0 = no decay.
3. TRANSITION_DURATION (1.0 to 4.0 seconds): Grace period after rotation.

STRATEGY:
- Iteration 1: alpha_nc=0.96, alpha_bc=0.995 (balanced starting point)
- Later: If Ka2 accuracy is poor, increase alpha_bc toward 1.0.
         If scale factors are noisy, decrease alpha_nc toward 0.93.
...
"""
    ...
    try:
        ...
        print(f"\n{'='*60}\n[LLM Graded SCD Advisor - Iter {iteration}]\n{content}\n{'='*60}\n")

        anc = 0.96; abc = 0.995; td = 2.0
        m = re.search(r'ALPHA_NC:\s*([0-9.]+)', content)
        if m: anc = float(m.group(1))
        m = re.search(r'ALPHA_BC:\s*([0-9.]+)', content)
        if m: abc = float(m.group(1))
        m = re.search(r'TRANSITION_DURATION:\s*([0-9.]+)', content)
        if m: td = float(m.group(1))

        return {
            'alpha_nc': np.clip(anc, 0.85, 0.99),
            'alpha_bc': np.clip(abc, 0.95, 1.0),
            'transition_duration': np.clip(td, 0.5, 10.0)
        }
    except Exception as e:
        print(f"  [LLM] Error: {e}")
        return {'alpha_nc': 0.96, 'alpha_bc': 0.995, 'transition_duration': 2.0}
...
def run_calibration(imu1, pos0, ts, scd_mode=False, label=""):
    ...
    for it in range(iterations):
        ...
        if scd_mode:
            ...
            print(f"  [{label}] Iter {it+1}/{iterations} | alpha_nc={scd_params['alpha_nc']:.3f}, "
                  f"alpha_bc={scd_params['alpha_bc']:.3f}, trans_dur={scd_params['transition_duration']:.1f}s")
        else:
        ...
        for k in range(2*frq2, length-frq2, nn):
            ...
            if t1s>(0.2-ts/2):
                ...
                if is_static:
                    ...
                    # === GRADED SCD: Different alphas for different physics ===
                    if scd_mode and scd_params and not scd_applied_this_phase:
                        if time_since_rot_stop >= scd_params['transition_duration']:
                            a_nc = scd_params['alpha_nc']   # Strong: noise pipeline
                            a_bc = scd_params['alpha_bc']   # Gentle: physical coupling
                            P = kf['Pxk']
                            # 1. Nav↔Calib: STRONG decay (direct noise pipeline)
                            P[0:6, 12:36] *= a_nc;  P[12:36, 0:6] *= a_nc
                            # 2. Bias↔Calib: GENTLE decay (preserve db↔Ka2 bond)
                            P[6:12, 12:36] *= a_bc;  P[12:36, 6:12] *= a_bc
                            # 3. Nav↔Bias: NO decay (biases need full nav coupling)
        ...
        if scd_mode and n_zupt > 0:
            ...
            prev_stats = {
                ...
                'ka2_vol': float(np.mean(np.std(X_arr[:, 27:30], axis=0))),
                'n_zupt': n_zupt, 'n_scd': n_scd,
            }
            print(f"    Applied Graded SCD to {n_scd} stationary phases")
