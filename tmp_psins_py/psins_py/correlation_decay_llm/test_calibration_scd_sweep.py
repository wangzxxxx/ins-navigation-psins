        ...
        for k in range(2*frq2, length-frq2, nn):
            ...
            if t1s>(0.2-ts/2):
                ...
                if is_static:
                    ...
                    # === SCD: Pre-Rotation Sweeping ===
                    if scd_mode and scd_params:
                        # 1. At the exact moment the transition grace period ends, save a snapshot of P
                        #    This P contains the good signal from the transition phase, before chronic noise sets in.
                        if time_since_rot_stop >= scd_params['transition_duration'] \
                                and time_since_rot_stop < scd_params['transition_duration'] + 0.2:
                            kf['P_base'] = np.copy(kf['Pxk'])
                
                # 2. When the stationary period ENDS (rotation is just about to start):
                #    Discard the chronically poisoned P matrix from the long stationary period.
                #    Instead, take the P_base snapshot, apply SCD decay to its cross-correlations,
                #    and push it back as the active P matrix for the upcoming rotation.
                if scd_mode and scd_params and not is_static and was_rotating and not scd_applied_this_phase:
                    if 'P_base' in kf:
