...
test_alpha_sweep.py
-------------------
Sweep alpha from 0.93 to 0.99 AND transition_duration from 1.0 to 3.0
to find the optimal SCD parameters.

Uses confirmed optimal structure:
  - P[nav, calib] *= alpha  (noise pipeline)
  - P[bias, calib] *= alpha (same alpha, equal ratio)
  - P[nav, bias] unchanged  (bias observability protection)
  - Applied ONCE per stationary phase
"""
import numpy as np
import sys, os, math
...
# =====================================================================
# Building Blocks (same as main script)
# =====================================================================
...
# =====================================================================
# Calibration Engine (parameterized)
# =====================================================================
def run_calibration(imu1, pos0, ts, alpha=None, trans_dur=2.0):
    """Run 5-iteration calibration. If alpha is None, standard KF."""
    eth=Earth(pos0)
        ...
        for k in range(2*frq2, length-frq2, nn):
            ...
            if t1s>(0.2-ts/2):
                ...
                if is_static:
                    ...
                    # SCD: confirmed optimal structure
                    if alpha is not None and not scd_applied:
                        if time_since_rot_stop >= trans_dur:
                            P = kf['Pxk']
                            P[0:6, 12:36] *= alpha;  P[12:36, 0:6] *= alpha   # nav<->calib
                            P[6:12, 12:36] *= alpha;  P[12:36, 6:12] *= alpha  # bias<->calib
                            # nav<->bias: NOT decayed
                            scd_applied = True
...
def score(clbt, clbt_truth, noisy_clbt):
    """Count how many parameters SCD beats Noisy KF."""
    ...
    wins = 0; total_err_scd = 0; total_err_noisy = 0
    details = []
    for nm,tr,ge in params:
        ...
        if es < eb_: wins += 1
        total_err_scd += es; total_err_noisy += eb_
        details.append((nm, tr, eb_, es))
    return wins, total_err_scd, total_err_noisy, details
...
def main():
    ...
    print("\n[Baseline] Noisy Standard KF...")
    noisy_clbt = run_calibration(imu_noisy, pos0, ts, alpha=None)

    # =====================================================================
    # SWEEP 1: Alpha sweep with fixed trans_dur=2.0
    # =====================================================================
    alphas = [0.93, 0.94, 0.95, 0.96, 0.97, 0.975, 0.98, 0.985, 0.99, 0.995]
    print("\n" + "="*80)
    print("ALPHA SWEEP (trans_dur=2.0s fixed)")
    print("="*80)

    best_alpha = 0.98; best_wins = 0
    alpha_results = []

    for a in alphas:
        print(f"\n  alpha={a:.3f} ...", end="", flush=True)
        scd_clbt = run_calibration(imu_noisy, pos0, ts, alpha=a, trans_dur=2.0)
        wins, err_scd, err_noisy, details = score(scd_clbt, clbt_truth, noisy_clbt)
        avg_err = err_scd / 30
        print(f"  WINS: {wins}/30  (avg err: {avg_err:.2f}%)")
        alpha_results.append((a, wins, avg_err))
        if wins > best_wins:
            best_wins = wins; best_alpha = a

    print(f"\n>> BEST ALPHA: {best_alpha:.3f} with {best_wins}/30 wins")

    # =====================================================================
    # SWEEP 2: Transition duration sweep with best alpha
    # =====================================================================
    trans_durs = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]
    print("\n" + "="*80)
    print(f"TRANSITION DURATION SWEEP (alpha={best_alpha:.3f} fixed)")
    print("="*80)

    best_td = 2.0; best_td_wins = 0
    td_results = []

    for td in trans_durs:
        print(f"\n  trans_dur={td:.1f}s ...", end="", flush=True)
        scd_clbt = run_calibration(imu_noisy, pos0, ts, alpha=best_alpha, trans_dur=td)
        wins, err_scd, err_noisy, details = score(scd_clbt, clbt_truth, noisy_clbt)
        avg_err = err_scd / 30
        print(f"  WINS: {wins}/30  (avg err: {avg_err:.2f}%)")
        td_results.append((td, wins, avg_err))
        if wins > best_td_wins:
            best_td_wins = wins; best_td = td

    print(f"\n>> BEST TRANSITION_DURATION: {best_td:.1f}s with {best_td_wins}/30 wins")
    ...
    print("FINAL SUMMARY")
    print("="*80)
    print(f"\nAlpha Sweep Results (trans_dur=2.0s):")
    print(f"  {'Alpha':>8}  {'Wins':>6}  {'Avg Err%':>10}")
    print(f"  {'-'*28}")
    for a, w, e in alpha_results:
        marker = " <<< BEST" if w == best_wins else ""
        print(f"  {a:>8.3f}  {w:>4}/30  {e:>9.2f}%{marker}")

    print(f"\nTransition Duration Sweep Results (alpha={best_alpha:.3f}):")
    print(f"  {'TD (s)':>8}  {'Wins':>6}  {'Avg Err%':>10}")
    print(f"  {'-'*28}")
    for td, w, e in td_results:
        marker = " <<< BEST" if w == best_td_wins else ""
        print(f"  {td:>8.1f}  {w:>4}/30  {e:>9.2f}%{marker}")

    print(f"\n>> OPTIMAL CONFIG: alpha={best_alpha:.3f}, trans_dur={best_td:.1f}s")
    print(f">> SCORE: {best_td_wins}/30 wins")

    # Run final optimal config and print detailed table
    print("\n" + "="*80)
    print(f"DETAILED RESULTS: alpha={best_alpha:.3f}, trans_dur={best_td:.1f}s")
    print("="*80)
    final_clbt = run_calibration(imu_noisy, pos0, ts, alpha=best_alpha, trans_dur=best_td)
    clean_clbt = run_calibration(imu_clean, pos0, ts, alpha=None)
    _, _, _, details = score(final_clbt, clbt_truth, noisy_clbt)
    
    print(f"\n{'Param':<10}{'Truth':>14}{'Noisy Err%':>12}{'SCD Err%':>12}  {'Win':>6}")
    print("-"*56)
    for nm, tr, eb_, es in details:
        w = "SCD" if es < eb_ else "Noisy"
        print(f"{nm:<10}{tr:>+14.6e}{eb_:>11.2f}%{es:>11.2f}%  {w:>6}")
