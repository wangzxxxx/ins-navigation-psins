        ...
        for k in range(2*frq2, length-frq2, nn):
            ...
            if t1s>(0.2-ts/2):
                ...
                if is_static:
                    ...
                    # === SCD: Prevention + Continuous Bleeding ===
                    if scd_mode and scd_params:
                        if time_since_rot_stop >= scd_params['transition_duration']:
                            P = kf['Pxk']
                            
                            if not scd_applied_this_phase:
                                # 1. PREVENTION: One-time moderate decay right after the transition grace period
                                a_sf = scd_params['alpha_sf']
                                # Decay nav↔calib cross-correlations (the main noise pipeline)
                                P[0:6, 12:36] *= a_sf;  P[12:36, 0:6] *= a_sf
                                scd_applied_this_phase = True
                            else:
                                # 2. BLEEDING: Tiny continuous decay to strictly prevent the pipeline from reopening
                                # Even when stationary, process noise Q injects tiny uncertainties that can re-entangle states.
                                a_bleed = 0.999  # Very gentle, ~18% over a 10s stationary phase (200 ZUPTs)
                                P[0:6, 12:36] *= a_bleed;  P[12:36, 0:6] *= a_bleed
