...
# LLM SCD Advisor
# =====================================================================
def get_llm_scd_params_per_rotation(iteration, stats):
    """
    LLM decides the correlation decay parameters dynamically for the UPCOMING stationary phase,
    based on the exact kinematics and state shifts of the rotation that JUST finished.
    """
    if client is None:
        return {'alpha_sf': 0.98, 'alpha_bias': 1.0, 'transition_duration': 2.0}

    stats_str = f"""
Rotation just finished (Phase {stats['phase']}/18):
  - Duration: {stats['dur']:.2f} seconds
  - Angle Changed: {stats['dtheta']:.2f} degrees
  - Velocity Changed: {stats['dv']:.4f} m/s
  - State Shift (Bias vector norm): {stats['bias_shift']:.4e}
  - State Shift (ScaleFactor vector norm): {stats['sf_shift']:.4e}
"""

    system_prompt = """You are tuning a Selective Correlation Decay (SCD) mechanism for IMU calibration IN REAL-TIME.

MECHANISM: When the system becomes stationary, after a 'transition grace period', the filter cross-correlations are decayed to cut the noise pipeline:
  P[nav, calib] *= alpha_sf
(Alpha_bias is fixed at 1.0 because decaying bias correlations destroys stability).

PARAMETERS TO TUNE:
1. ALPHA_SF (0.85 to 1.0): Per-stationary-phase decay. 
   0.98 = gentle cut (default, safe)
   0.95 = strong cut (if you see huge state shifts during rotation that look unstable)
   1.0 = no decay (if rotation was tiny, e.g. < 5 degrees)
2. TRANSITION_DURATION (1.0 to 4.0 seconds): Grace period after rotation stops before applying the cut.
   Larger rotations (e.g. 90, 180 deg) excite more modes and need longer settling time (e.g. 2.0s to 3.0s).
   Small rotations need shorter settling time (e.g. 1.0s).

OUTPUT FORMAT:
REASONING: [brief 1 sentence analysis of this specific rotation]
ALPHA_SF: [value]
TRANSITION_DURATION: [seconds]
"""

    user_prompt = f"This is iteration {iteration}/5.\n{stats_str}\nProvide parameters for the imminent stationary phase."
    ...
    try:
        ...
        print(f"    [Rot {stats['phase']:02d}] dTheta: {stats['dtheta']:>6.1f} deg | LLM: {content.replace(chr(10), ' | ')}")
...
def run_calibration(imu1, pos0, ts, scd_mode=False, label=""):
    ...
    for it in range(iterations):
        scd_params = None
        if scd_mode:
            print(f"\n  [{label}] Iter {it+1}/{iterations} (Per-Rotation LLM Adaptive)")
        else:
            print(f"\n  [{label}] Iter {it+1}/{iterations}")
        ...
        # Trackers for the active rotation
        rot_wm_acc = np.zeros(3)
        rot_vm_acc = np.zeros(3)
        rot_frames = 0
        X_before_rot = np.zeros(36)
        ...
        for k in range(2*frq2, length-frq2, nn):
            ...
            if t1s>(0.2-ts/2):
                ...
                if not is_static:
                    ...
                    # Accumulate kinematic data correctly (just the current step's delta)
                    rot_wm_acc += np.sum(wm, axis=0)
                    rot_vm_acc += np.sum(vm, axis=0)
                    rot_frames += 1
                else:
                    if was_rotating:
                        ...
                        # Just finished rotation, calculate stats & call LLM
                        dur_rot = rot_frames * nts
                        dtheta_deg = np.linalg.norm(rot_wm_acc) / glv.deg
                        dv_mps = np.linalg.norm(rot_vm_acc)
                        
                        dx = kf['xk'] - X_before_rot
                        bias_shift = np.linalg.norm(dx[6:12])
                        sf_shift = np.linalg.norm(dx[12:30])
                        
                        rot_stats = {
                            'phase': rot_phase_count,
                            'dur': dur_rot,
                            'dtheta': dtheta_deg,
                            'dv': dv_mps,
                        ...
                        if scd_mode:
                            scd_params = get_llm_scd_params_per_rotation(it + 1, rot_stats)
                    else:
                ...
                if is_static:
                    ...
                    if scd_mode and scd_params and not scd_applied_this_phase:
                        if time_since_rot_stop >= scd_params['transition_duration']:
                            ...
                            P = kf['Pxk']
                            # Decay nav↔calib cross-correlations ONLY
        ...
        if scd_mode and n_zupt > 0:
            print(f"    Applied Dynamic SCD to {rot_phase_count} stationary phases")
