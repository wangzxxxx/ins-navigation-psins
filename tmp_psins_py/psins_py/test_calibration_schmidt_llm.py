...
{state_info_str}
    ...
    try:
        ...
        if indices_match:
            ...
            # Strict Complete Masking: Gain = 0.0 for selected unobservable states
            BETA_SUPPRESS = 0.0
            for idx in indices:
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if np.linalg.norm(ww) / ts < 20 * glv.dph:
                    if llm_mode:
                        # Schmidt-Kalman (Consider Filter) Update with Joseph Form
                        # 1. 正常计算原本的最优增益
                        Pxy = kf['Pxk'] @ kf['Hk'].T
                        Py0 = kf['Hk'] @ Pxy
                        Py  = Py0 + kf['Rk']
                        try:
                            Kk = Pxy @ np.linalg.inv(Py)
                        except np.linalg.LinAlgError:
                            Kk = Pxy @ np.linalg.pinv(Py)
                        
                        # 2. 对不可观测的参数强制清零增益矩阵（物理隔离噪声污染）
                        for idx_state in range(12, 36):
                            Kk[idx_state, :] *= current_mask[idx_state]
                        ...
                        # 4. 必须使用 Joseph Form 来更新协方差，确保正定性与完全隔离参数的方差不受损
                        I_KH = np.eye(36) - Kk @ kf['Hk']
                        kf['Pxk'] = I_KH @ kf['Pxk'] @ I_KH.T + Kk @ kf['Rk'] @ Kk.T
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plots_schmidt_llm')
