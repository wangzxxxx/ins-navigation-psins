...
def get_llm_r_scalar(history_context, kf_state, kf_diag_p):
    """
    调用 LLM，不仅根据运动历史，还根据当前的滤波器状态和方差，
    自主评判当前量测的“可信度”，输出一个测量噪声放大倍数（R_SCALAR）。
    """
    ...
    system_rules = """
    You are an autonomous expert Kalman Filter covariance scheduler.
    The IMU calibration filter is attempting to estimate 36 parameters.
    Indices 12-20: Gyro scale/misalignment
    Indices 21-29: Accelerometer scale/misalignment
    
    CRITICAL CONCEPT: The filter performs zero-velocity updates (ZUPT) when STATIONARY. 
    However, during stationary periods, rotation-dependent parameters (like scale factors and misalignments) are theoretically UNOBSERVABLE. If the filter tries to aggressively update them anyway, it simply absorbs random sensor noise, which ruins calibration accuracy.

    *YOUR AUTONOMOUS TASK*:
    You will be given the CURRENT ACTUAL NUMERICAL STATE of the filter (Value and Standard Deviation for indices 12-35), along with the preceding motion history.
    1. Look at the preceding rotation history and the current physical state.
    2. Look at the `Value` and `StdDev`. If unobservable parameters are drifting significantly (Values getting unreasonably large) while stationary, the filter is trusting the stationary velocity zero-measurements too much and absorbing pure noise.
    
    Instead of rigidly forbidding specific states, your job is to tell the filter to DISTRUST its current measurements by INFLATING the Measurement Noise Matrix (R matrix).
    Output a single magnification multiplier `R_SCALAR` (a float between 1.0 and 1000.0).
    - If you are extremely confident the measurements are pure noise leaking into unobservable states, output a high scalar (e.g., 200.0 or 500.0).
    - If the states look stable or motion is happening, output 1.0.
    
    Please output your response in TWO sections:
    1. REASONING: Explain your judgment based on both the motion history AND the numerical state.
    2. R_SCALAR: Only output the precise float multiplier.

    Example format:
    REASONING: The turntable has been stationary for a while. The scale factors (Index 12, 21) are drifting and their StdDevs are stagnant, meaning the zero-velocity measurement is just feeding noise into them. I will inflate the R matrix to suppress this false learning.
    R_SCALAR: 150.0
    """
...
{state_info_str}
    ...
    Tell me how much to inflate the Measurement Noise R matrix AT THIS VERY MOMENT to prevent noise from leaking into unobservable states.
    (Output your response in the requested REASONING and R_SCALAR format).
    """

    if is_first_llm_call:
        messages = [
            {"role": "system", "content": "You are a precise Kalman filter R-matrix scheduler."},
    ...
    try:
        ...
        # Parse scalar
        match = re.search(r'R_SCALAR:\s*([0-9.]+)', content)
        if match:
            scalar = float(match.group(1))
            return min(max(scalar, 1.0), 1000.0) # bound between 1x and 1000x
        return 1.0
    except Exception as e:
...
def run_calibration(imu1, pos0, ts, llm_mode=False, label=""):
    ...
    for it in range(iterations):
        ...
        # 默认 R_SCALAR 倍率为 1.0 (不放缩)
        current_r_scalar = 1.0
        ...
        # 统计静止时间，以便在静止中期再向 LLM 要 R_SCALAR
        stationary_time = 0.0
        llm_called_this_phase = False
        
        # 统计掩码被实际应用的次数（调试用）
        r_scalar_applied_count = 0
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if current_motion_status != last_motion_status:
                ...
                if current_motion_status == "STATIONARY":
                    stationary_time = 0.0
                    llm_called_this_phase = False
                    current_r_scalar = 1.0 # 刚停下时不急着放缩，先吃些真实动态
                else: # ROTATING
                    current_r_scalar = 1.0 # 旋转时放开增益
            ...
            if current_motion_status == "STATIONARY":
                stationary_time += nts
                # 在静止了 5 秒钟后，系统状态比较稳定了，向 LLM 询问对量测噪声的专家分析
                if stationary_time > 5.0 and llm_mode and not llm_called_this_phase:
                    if it == 0:
                        recent_history = "\n".join(motion_history[-5:])
                        # 向 LLM 索要 R 的放大倍数
                        current_r_scalar = get_llm_r_scalar(recent_history, kf['xk'], np.diag(kf['Pxk']))
                        state_key = "_".join([h.split("->")[1].strip() for h in motion_history[-4:]])
                        global_llm_mask_cache[state_key] = current_r_scalar
                    else:
                        state_key = "_".join([h.split("->")[1].strip() for h in motion_history[-4:]])
                        if state_key in global_llm_mask_cache:
                            current_r_scalar = global_llm_mask_cache[state_key]
                        else:
                            current_r_scalar = 1.0
            ...
            if t1s > (0.2 - ts / 2):
                ...
                if np.linalg.norm(ww) / ts < 20 * glv.dph:
                    if llm_mode and current_r_scalar > 1.0:
                        # Adaptive R Matrix Method
                        original_Rk = np.copy(kf['Rk'])
                        kf['Rk'] = kf['Rk'] * current_r_scalar # 临时放大测量噪声
                        
                        kf = kfupdate(kf, yk=vn, TimeMeasBoth='M') # Pxk and xk are perfectly updated here
                        
                        kf['Rk'] = original_Rk # 恢复以防后续逻辑需要原值
                        r_scalar_applied_count += 1
                    else:
        ...
        if llm_mode:
            print(f"    [Debug] Over this iteration, R-matrix inflation was applied {r_scalar_applied_count} times.")
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plots_adaptive_rq_llm')
