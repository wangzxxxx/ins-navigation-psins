...
class EnhancedKalmanFilter(dict):
    ...
    def shadow_update(self, z_llm, H_llm, conf_ratio):
        """异步影子注入，直接映射相对标量协方差，解决协方差坍塌问题"""
        z_pred = H_llm @ self['xk']
        innovation = z_llm - z_pred[0]
        
        current_variance = float((H_llm @ self['Pxk'] @ H_llm.T)[0, 0])
        R_llm = current_variance * conf_ratio
    ...
    for it in range(iterations):
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s_base > (0.2 - ts / 2):
                ...
                # Log state and variance ONLY on final iteration
                if it == iterations - 1:
                            ...
                            for decision in decisions:
                                if decision.get("confidence") in shadow_manager.confidence_map:
                                    ...
                                    if r_llm is not None:
                                        ...
                                        print(f"      -> LLM Authorized! Target Index {idx}, z_llm: {val:.2e}, R: {r_llm:.1e} ({conf})")
                                        print(f"      -> Pre-inject Variance (idx {idx}): {var_pre:.2e}")
                                    
                                        # 建立 H_llm 进行更新
                                        H_llm = np.zeros((1, len(kf_shadow['xk'])))
                                        H_llm[0, idx] = 1.0
                                    
                                        kf_shadow.shadow_update(val, H_llm, r_llm)
