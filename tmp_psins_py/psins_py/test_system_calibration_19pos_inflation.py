...
from psins_py.shadow_manager_inflation import ShadowSimulationManager
...
class EnhancedKalmanFilter(dict):
    ...
    def attention_update(self, target_idx, inflation_factor):
        """自适应方差膨胀：放弃绝对值直接注入，改为放大信度极低的被困参数方差"""
        # Exert attention multiplying the covariance matrix diagonal for the targeted index
        self['Pxk'][target_idx, target_idx] *= inflation_factor
...
def main():
    ...
    for it in range(iterations):
        ...
        for k in range(2 * frq2, length - frq2, nn):
            ...
            if t1s_base > (0.2 - ts / 2):
                t1s_base = 0.0
                ww = np.mean(imu1[k-frq2:k+frq2+1, 0:3], axis=0) 
                if np.linalg.norm(ww) / ts < 150 * glv.dph:  # Zero-velocity physical stationary update
                    kf_base.kf_update(yk=vn_base, TimeMeasBoth='M')
                
                # Log state and variance unconditionally for continuous multi-iteration tracking
            ...
            if t1s_shadow > (0.2 - ts / 2):
                t1s_shadow = 0.0
                ww = np.mean(imu1[k-frq2:k+frq2+1, 0:3], axis=0) 
                if np.linalg.norm(ww) / ts < 150 * glv.dph:
                ...
                if np.linalg.norm(ww) / ts < 150 * glv.dph:
                    ...
                    if enable_llm_shadow and it == 0 and shadow_trigger_count > 0 and shadow_trigger_count % 100 == 0:
                        ...
                        if decisions:
                            ...
                            for decision in decisions:
                                if decision.get("confidence") in shadow_manager.confidence_map:
                                    ...
                                    inflation_factor = shadow_manager.confidence_map[conf]
                                
                                    if inflation_factor is not None:
                                        idx = decision.get("target_index", 0)
                                    
                                        var_pre = kf_shadow['Pxk'][idx, idx]
                                        print(f"      -> Attention LLM Authorized! Target Index {idx}, Factor: {inflation_factor:.1f}x ({conf})")
                                        print(f"      -> Pre-inflation Variance (idx {idx}): {var_pre:.2e}")
                                    
                                        kf_shadow.attention_update(idx, inflation_factor)
                                    
                                        var_post = kf_shadow['Pxk'][idx, idx]
                                        print(f"      -> Post-inflation Variance (idx {idx}): {var_post:.2e}")
                                    else:
    ...
    out_dir_states = 'plots_state_svg'
    ...
    for i in range(43):
        ...
        name = state_names[i] if i < len(state_names) else f"State_{i}"
        plt.title(f"State Value Convergence for {name} (Index {i})")
        plt.xlabel("Maneuver Time (s)")
    ...
    print("="*85)
    print(f"{'State Name':<12} | {'True Val':<15} | {'Base Est':<15} | {'Shadow Est':<15} | {'Base Err':<12}")
    print("-" * 85)
    
    def print_comp(name, true_val, base_val, shadow_val):
        b_actual = -base_val
        s_actual = -shadow_val
        err = abs(true_val - b_actual)
        print(f"{name:<12} | {true_val:>15.6e} | {b_actual:>15.6e} | {s_actual:>15.6e} | {err:>12.6e}")
