...
try:
    ...
    if api_key and base_url:
        ...
        print(f"[LLM Outer-Loop] Client initialized: model={model_name}")
    else:
        client = None
except Exception as e:
    client = None

def get_llm_hyperparams(perf_report):
    if client is None:
        return 1.0, 1.0, 1.0
    
    system_rules = """
    You are an autonomous Hyperparameter Tuning Expert for a 36-state Kalman Filter IMU Calibration system.
    Your task is to analyze the performance metrics from the PREVIOUS calibration iteration and output MULTIPLIERS for the upcoming iteration's initial covariance matrices (Q and R).

    1. Q_BIAS_SCALAR: Multiplier for the Process Noise (Q) of Bias parameters (states 6-11). If bias estimation is too slow, increase it; if they are too volatile, decrease it.
    2. Q_SF_SCALAR: Multiplier for the Process Noise (Q) of Scale Factor & Misalignment parameters (states 12-35). If scale factors are chasing noise (high volatility while stationary), you must shrink this globally (e.g., 0.1 or 0.01) to force stiffness. If they are failing to reach the truth, increase it.
    3. R_SCALAR: Multiplier for global Measurement Noise (R). If the input data has huge initial noise causing wild spikes across the board, increase R_SCALAR globally (e.g., 2.0 to 10.0) so the filter trusts the Zero-Velocity constraints less during high-vibration phases.
    
    Format:
    REASONING: The scale factors (Indices 12-29) displayed high volatility due to noise ...
    Q_BIAS_SCALAR: 1.0
    Q_SF_SCALAR: 0.1
    R_SCALAR: 5.0
    """
    ...
    user_prompt = f"""
    Here is the summary report of the filter's performance during the last calibration macro-iteration:
    ======================================
{perf_report}
    ======================================
    Based on this, what global multipliers should we apply to the Q and R matrices for the Next Iteration?
    """
    ...
    try:
        ...
        q_bias = 1.0; q_sf = 1.0; r_s = 1.0
        match_bias = re.search(r'Q_BIAS_SCALAR:\s*([0-9.]+)', content)
        match_sf = re.search(r'Q_SF_SCALAR:\s*([0-9.]+)', content)
        match_r = re.search(r'R_SCALAR:\s*([0-9.]+)', content)
        
        if match_bias: q_bias = float(match_bias.group(1))
        if match_sf:   q_sf   = float(match_sf.group(1))
        if match_r:    r_s    = float(match_r.group(1))
        
        # Safe bounds
        return np.clip(q_bias, 0.001, 100.0), np.clip(q_sf, 0.000001, 10.0), np.clip(r_s, 0.01, 1000.0)
    except Exception as e:
        print(f"    [LLM] Error: {e}")
        return 1.0, 1.0, 1.0
...
#  [30:33] rx    [33:36] ry
# ═══════════════════════════════════════════════════════════════
def clbtkfinit_36(nts, q_bias_s=1.0, q_sf_s=1.0, r_s=1.0):
    n = 36
    kf = {'nts': nts, 'n': n, 'm': 3}
    ...
    # Apply LLM Multipliers to Process Noise
    for i in range(6, 12):  kf['Qk'][i, i] *= q_bias_s
    for i in range(12, 36): kf['Qk'][i, i] *= q_sf_s

    # Apply LLM Multiplier to Measurement Noise
    kf['Rk'] = np.diag([0.001, 0.001, 0.001])**2 * r_s
...
def run_calibration(imu1, pos0, ts, n_states=36,
                    ...
                    llm_mode=False, label="Outer-Loop LLM"):
    eth  = Earth(pos0)
    ...
    iterations = 5 if llm_mode else 3
    ...
    for it in range(iterations):
        if llm_mode:
            print(f"  [{label}] Iteration {it+1}/{iterations} | Q_Bias: x{q_bias_s:.2f}, Q_SF: x{q_sf_s:.2e}, R: x{r_s:.2f}")
        else:
        ...
        if n_states == 48:
            ...
            kf = clbtkfinit_36(nts, q_bias_s, q_sf_s, r_s)
        ...
        if it != iterations - 1:
            ...
            # --- LLM Reporting and Request ---
            if llm_mode:
                # Analyze the last 20% of the trace from this iteration to gauge steady-state volatility
                analyzed_len = max(100, len(X_trace) // 5)
                recent_X = np.array(X_trace[-analyzed_len:])
                recent_P = np.array(P_trace[-analyzed_len:])
                
                volatility_sf = np.mean(np.std(recent_X[:, 12:30], axis=0)) # Drift/jitter in Scale Factors
                max_P_sf = np.mean(np.max(np.sqrt(recent_P[:, 12:30]), axis=0)) # Final Uncertainty remaining
                volatility_bias = np.mean(np.std(recent_X[:, 6:12], axis=0))
                
                report = f"""
                Summary of Iteration {it+1}:
                - Gyro/Accel Scale Factors Mean Volatility (std dev over last phase): {volatility_sf:.4e}
                - Gyro/Accel Scale Factors Peak Uncertainty (sqrt(P)): {max_P_sf:.4e}
                - Gyro/Accel Biases Mean Volatility: {volatility_bias:.4e}
                - Current Settings Used: Q_BIAS_SCALAR={q_bias_s}, Q_SF_SCALAR={q_sf_s}, R_SCALAR={r_s}
                """
                
                q_bias_s, q_sf_s, r_s = get_llm_hyperparams(report)
...
def main():
    ...
    print("\n[B] Full noise (Standard KF, 3 Iters)...")
    res_B = run_calibration(imu_noisy, pos0, ts, n_states=36, label="36-state KF")

    # ── C: 外环全自动调优 ──
    print("\n[C] Full noise w/ AI Outer-Loop (LLM Tuner, 5 Iters)...")
    res_LLM = run_calibration(imu_noisy, pos0, ts, n_states=36, llm_mode=True, label="LLM Tuned")

    # 绘制对比图 (专门针对 36维)
    P_A, X_A = res_A[2], res_A[3]
    P_B, X_B = res_B[2], res_B[3]
    P_L, X_L = res_LLM[2], res_LLM[3]
    
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plots_hyperparam_llm')
    os.makedirs(out_dir, exist_ok=True)
    
    min_len = min(len(P_A), len(P_B))
    time_arr = np.arange(min_len) * (2 * ts)
    time_arr_L = np.arange(len(P_L)) * (2 * ts)
    ...
    for i in range(6, 36):
        ...
        # Uncertainty
        ax1 = axes[0]
        ax1.plot(time_arr, np.sqrt(P_A[:min_len, i]), color='blue', label='Clean', alpha=0.8)
        ax1.plot(time_arr, np.sqrt(P_B[:min_len, i]), color='red',  label='Noisy KF (Iter 3)', linestyle='--', linewidth=2)
        ax1.plot(time_arr_L, np.sqrt(P_L[:len(time_arr_L), i]), color='green', label='LLM Tuned (Iter 5)', linewidth=2, alpha=0.6)
        
        for b_idx in res_LLM[4][:-1]: 
            t_bound = b_idx * (2 * ts)
            if t_bound < max(time_arr_L):
        ...
        # Value
        ax2 = axes[1]
        ax2.plot(time_arr, X_A[:min_len, i], color='blue', label='Clean', alpha=0.8)
        ax2.plot(time_arr, X_B[:min_len, i], color='red', label='Noisy KF (Iter 3)', linestyle='--', linewidth=2)
        ax2.plot(time_arr_L, X_L[:len(time_arr_L), i], color='green', label='LLM Tuned (Iter 5)', linewidth=2, alpha=0.6)
    ...
    print(f"\nDone! Overriding other outputs. Plots saved to {out_dir}")
    return
