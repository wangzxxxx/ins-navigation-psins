...
OUT_PNG = WORKSPACE / 'tmp' / 'alignment_strategy_sweep' / 'fig_ch4_alignment_convergence_A_pub_2026-03-31.png'
OUT_JSON = WORKSPACE / 'tmp' / 'alignment_strategy_sweep' / 'ch4_alignment_convergence_A_2026-03-31.json'
...
mod = load_module('align_h24_convergenceA_20260331', SCRIPT_PATH)
acc18 = mod.load_acc18()
base12 = mod.load_base12()
glv = acc18.glv


def trace_single_pass(scale_active_from_start: bool, seed: int = 0, sample_stride_raw: int = 1000):
    np.random.seed(seed)
    ts = 0.01
    ...
    kf = mod.avnkfinit_24(
        ...
        enable_scale_states=scale_active_from_start,
    )

    vn = np.zeros(3)
    length = (len(imu_noisy) // nn) * nn
    trace = []
    for k in range(0, length, nn):
        wvm = imu_noisy[k:k + nn, 0:6]
    ...
    final_truth = att_truth[-1, 0:3]
    final_err = acc18.qq2phi(qnbi, acc18.a2qua(final_truth)) / glv.sec
    return {
        'trace': trace,
        'final_att_arcsec': [float(x) for x in final_err],
        'scale_active_from_start': bool(scale_active_from_start),
    }


def main():
    plain = trace_single_pass(True, seed=0)
    staged = trace_single_pass(False, seed=0)
    ...
    axes[0, 0].plot(tp, [row['roll_abs_arcsec'] for row in plain['trace']], label='plain24 iter1', linewidth=2.0)
    axes[0, 0].plot(ts, [row['roll_abs_arcsec'] for row in staged['trace']], label='staged24 iter1', linewidth=2.0)
    ...
    axes[0, 1].plot(tp, [row['pitch_signed_arcsec'] for row in plain['trace']], label='plain24 iter1', linewidth=2.0)
    axes[0, 1].plot(ts, [row['pitch_signed_arcsec'] for row in staged['trace']], label='staged24 iter1', linewidth=2.0)
    ...
    axes[1, 0].plot(tp, [row['yaw_abs_arcsec'] for row in plain['trace']], label='plain24 iter1', linewidth=2.0)
    axes[1, 0].plot(ts, [row['yaw_abs_arcsec'] for row in staged['trace']], label='staged24 iter1', linewidth=2.0)
    ...
    axes[1, 1].plot(tp, [row['norm_arcsec'] for row in plain['trace']], label='plain24 iter1', linewidth=2.0)
    axes[1, 1].plot(ts, [row['norm_arcsec'] for row in staged['trace']], label='staged24 iter1', linewidth=2.0)
    ...
    fig.suptitle('Single-pass alignment convergence under the same initial misalignment (seed0)')
    fig.tight_layout()
    fig.savefig(OUT_PNG, dpi=240, bbox_inches='tight')
    plt.close(fig)

    payload = {
        'plain24_iter1_seed0': plain,
        'staged24_iter1_seed0': staged,
        'figure': str(OUT_PNG),
        'note': 'Both methods start from the same coarse-alignment error; only the scale-state release policy differs.',
    }
