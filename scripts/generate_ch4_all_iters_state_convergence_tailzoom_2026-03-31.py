...
IN_JSON = WORKSPACE / 'tmp' / 'alignment_strategy_sweep' / 'ch4_all_iters_state_convergence_cleanview_2026-03-31.json'
OUT_PNG = WORKSPACE / 'tmp' / 'alignment_strategy_sweep' / 'fig_ch4_all_iters_state_convergence_tailzoom_pub_2026-03-31.png'
OUT_JSON = WORKSPACE / 'tmp' / 'alignment_strategy_sweep' / 'ch4_all_iters_state_convergence_tailzoom_2026-03-31.json'
CUT_S = 260.0
...
def main():
    ...
    plain = {k: filtered(v) for k, v in obj['plain24_seed0_cleanview'].items()}
    staged = {k: filtered(v) for k, v in obj['staged24_seed0_cleanview'].items()}
    ...
    for col, it in enumerate(['1', '2', '3', '4', '5']):
        ...
        for ridx, (key, ylabel) in enumerate(zip(row_keys, ylabels)):
            ...
            if ridx == 0:
                ax.set_title(f'iter={it} (tail zoom)')
            if ridx == 0 and col == 4:
                ax.legend(frameon=False, loc='upper right')

    fig.suptitle('Tail-zoomed state convergence after the large initial transient (t ≥ 260 s)')
    fig.tight_layout()
    ...
    payload = {
        ...
        'figure': str(OUT_PNG),
        'cut_s': CUT_S,
        'plain24_tailzoom': plain,
        'staged24_tailzoom': staged,
        'note': 'This zoomed figure keeps only the late convergence segment to make the method differences visible after the large initial transient.',
    }
