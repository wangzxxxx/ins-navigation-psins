#!/usr/bin/env python3
"""Controlled GM-drift robustness check for Chapter 4 DAR plain24 vs staged24."""
...
OUT_JSON = OUT_DIR / 'ch4_plain24_staged24_truth_gm_robustness_2026-03-31.json'
OUT_MD = OUT_DIR / 'ch4_plain24_staged24_truth_gm_robustness_2026-03-31.md'
OUT_CSV = OUT_DIR / 'ch4_plain24_staged24_truth_gm_robustness_table_2026-03-31.csv'
...
SEEDS = [0, 1, 2, 3, 4]
CONDITIONS = ['baseline', 'mild_gm', 'stronger_gm']
...
def load_base12():
    global _BASE12
    if _BASE12 is None:
        _BASE12 = load_module('alignvn_base12_truth_gm_robust_20260331', BASE12_PATH)
    return _BASE12
...
def load_hybrid24():
    ...
    if _HYBRID24 is None:
        _HYBRID24 = load_module('alignvn_hybrid24_truth_gm_robust_20260331', HYBRID24_PATH)
    return _HYBRID24
...
def load_gm_helper():
    ...
    if _GM_HELPER is None:
        _GM_HELPER = load_module('alignvn_truth_gm_helper_20260331', GM_HELPER_PATH)
    return _GM_HELPER
...
def build_markdown(payload: dict[str, Any]) -> str:
    lines = [
        '# Chapter 4 DAR plain24 vs staged24 truth-side GM robustness (2026-03-31)',
        '',
        '## What was added',
    ...
    lines.extend([
        ...
        f'- script: `{SCRIPTS_DIR / "alignvn_dar_hybrid24_truth_gm_robustness_py_2026-03-31.py"}`',
        f'- json: `{OUT_JSON}`',
        f'- md: `{OUT_MD}`',
        f'- csv: `{OUT_CSV}`',
        '',
...
def main() -> None:
    ...
    payload = {
        'meta': {
            'date': '2026-03-31',
            'purpose': 'controlled robustness check for Chapter 4 DAR plain24 vs staged24 under added truth-side GM drift',
            'seeds': SEEDS,
