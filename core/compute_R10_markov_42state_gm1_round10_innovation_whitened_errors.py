...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round10_innovation_whitened.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R10_markov_42state_gm1_round10_innovation_whitened_param_errors.json'
...
meth = load(METHOD, 'meth_R10')
src = load(SRC, 'src_R10')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R10_markov_42state_gm1_round10_innovation_whitened','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
