...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round17_static_consensus_frozen_weakrefine.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R17_markov_42state_gm1_round17_static_consensus_frozen_weakrefine_param_errors.json'
...
meth = load(METHOD, 'meth_R17')
src = load(SRC, 'src_R17')
res = meth.run_method()
clbt = res[0]
...
OUT.write_text(json.dumps({'method':'R17_markov_42state_gm1_round17_static_consensus_frozen_weakrefine','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
