...
def parse_args():
    ...
    parser.add_argument('--noise-scale', type=float, default=1.0 / 3.0)
...
def build_output_paths(noise_scale: float):
    suffix = make_suffix(noise_scale)
    out_baseline = RESULTS_DIR / f'M_markov_42state_gm1_shared_{suffix}_param_errors.json'
    out_r61 = RESULTS_DIR / f'R61_42state_gm1_round61_h_scd_state20_microtight_commit_shared_{suffix}_param_errors.json'
    out_compare = RESULTS_DIR / f'compare_baseline_vs_round61_shared_{suffix}.json'
    return suffix, out_baseline, out_r61, out_compare
...
def main():
    ...
    suffix, out_baseline, out_r61, out_compare = build_output_paths(noise_scale)

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    source_mod = load_module(f'markov_pruned_source_shared_{suffix}', str(SOURCE_FILE))
    r53_mod = load_module(f'markov_r53_shared_{suffix}', str(R53_METHOD_FILE))
    r61_mod = load_module(f'markov_r61_shared_{suffix}', str(R61_METHOD_FILE))
    compute_r61_mod = load_module(f'compute_r61_shared_{suffix}', str(COMPUTE_R61_FILE))
    ...
    baseline_label = f'42-GM1-BASELINE-SHARED-{suffix.upper()}'
    ...
    baseline_payload = compute_payload(
        ...
        method_file=f'baseline_direct_run_calibration_shared_{suffix}',
        extra={
            'noise_scale': noise_scale,
    ...
    r61_label = f'42-GM1-R61-SHARED-{suffix.upper()}'
    ...
    for key in params:
        name = key[0]
        b = baseline_payload['param_errors'][name]['pct_error']
        r = r61_payload['param_errors'][name]['pct_error']
        comparison['all_params'][name] = {
