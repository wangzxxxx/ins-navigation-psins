...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_round29_dominantaxis_trustmatch.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R29_markov_42state_gm1_round29_dominantaxis_trustmatch_param_errors.json'
...
meth = load(METHOD, 'meth_R29')
src = load(SRC, 'src_R29')
res = meth.run_method()
clbt = res[0]
extra = res[4]
...
OUT.write_text(json.dumps({'method': 'R29_markov_42state_gm1_round29_dominantaxis_trustmatch', 'summary': summary, 'rows': rows, 'extra': extra}, ensure_ascii=False, indent=2))
print(f'WROTE {OUT}')
