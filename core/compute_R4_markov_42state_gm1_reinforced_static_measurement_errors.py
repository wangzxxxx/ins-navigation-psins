...
METHOD = ROOT / 'psins_method_bench' / 'methods' / 'markov' / 'method_42state_gm1_reinforced_static_measurement.py'
SRC = ROOT / 'tmp_psins_py' / 'psins_py' / 'test_calibration_markov_pruned.py'
OUT = ROOT / 'psins_method_bench' / 'results' / 'R4_markov_42state_gm1_reinforced_static_measurement_param_errors.json'
...
OUT.write_text(json.dumps({'method':'R4_markov_42state_gm1_reinforced_static_measurement','summary':summary,'rows':rows}, ensure_ascii=False, indent=2))
print(json.dumps({'summary':summary,'first_rows':rows[:8]}, ensure_ascii=False))
print(f'WROTE {OUT}')
