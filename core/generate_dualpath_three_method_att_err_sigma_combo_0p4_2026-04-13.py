...
CUSTOM_PREVIEW_SCRIPT = WORKSPACE / 'psins_method_bench' / 'scripts' / 'generate_dualpath_three_method_att_err_preview_0p4_2026-04-13.py'
OUT_DIR = WORKSPACE / 'tmp' / 'psins_dualpath_three_method_att_err_sigma_combo_0p4_2026-04-13'
...
custom = load_module('dualpath_combo_custom_0p4_20260413', CUSTOM_PREVIEW_SCRIPT)
aep = custom.base
bp = aep.bp
...
def render_combo(out_svg: Path, out_png: Path, axis_label: str, err_series, sigma_series):
    ...
    parts.append(f'<text x="18" y="26" class="title">{escape(axis_label)} · noise ×0.4 · seed=0</text>')
    ...
    draw_panel(parts, panels[1], f'Tail-detail zoom · round-{last_round_idx} last 100 s', tail_ticks, nice_ticks(tail_ymin, tail_ymax, 5), mx2, my2, 'time (s)', 'arcsec', zero_y=zero2, iter_bounds=tail_iter)
...
def main():
    ...
    aep.OUTER_ITERS = OUTER_ITERS
    bp.OUTER_ITERS = OUTER_ITERS
    bp.build_shared_dual_dataset = custom.build_shared_dual_dataset_custom_noise_0p4
    aep.bp.build_shared_dual_dataset = custom.build_shared_dual_dataset_custom_noise_0p4

    shared = custom.build_shared_dual_dataset_custom_noise_0p4()
    err_groups = [
        aep.run_scale18_att_err(shared),
        aep.run_plain24_att_err(shared),
        aep.run_purescd_att_err(shared),
    ]
    state_groups = [
        bp.trace_scale18_first3(shared),
        bp.trace_plain24_first3(shared),
        bp.trace_purescd24_first3(shared),
    ]
    sigma_map = {
    ...
    summary = {
        'task': 'dualpath_three_method_att_err_sigma_combo_0p4_2026_04_13',
        'note': 'Three-panel combo for 0.4x custom noise: full error, tail zoom, and approximate covariance convergence (sigma(phi_x/y/z)).',
    ...
    for axis in ['att_err_x', 'att_err_y', 'att_err_z']:
        ...
        sigma_series = [
            ...
            for gk in ['g2_scaleonly_rotation', 'g3_markov_rotation', 'g4_scd_rotation']
        ]
